...
void RunRuntimeTests(TestContext* ctx) {
  ...
  auto if_val = EvalStmt("if (false) 10 else 7", env);
  ...
  auto nested = EvalStmt("if (true) { if (false) 9 else 8 } else 7", env);
  ExpectNear(Unwrap(nested.value, "nested_if", ctx).number, 8.0, "nested_if", ctx);
  auto guard = EvalStmt("if (true) 5 else mod(1, 0)", env);
  ...
  ExpectNear(Unwrap(loop.value, "while_basic_increment", ctx).number, 3.0, "while_basic_increment",
             ctx);
  auto zero_loop = EvalStmt("{ j = 0; while (false) { j = j + 1; }; j }", env);
  ...
  auto with_break =
      EvalStmt("{ k = 0; while (true) { k = k + 1; if (k != 2) { } else { break; } }; k }", env);
