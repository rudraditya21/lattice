...
kernel void lattice_svd(const device scalar_t* a [[buffer(0)]],
                        device scalar_t* u [[buffer(1)]],
                        device scalar_t* s [[buffer(2)]],
                        device scalar_t* v [[buffer(3)]],
    ...
    if (lattice_abs(ata01) < (scalar_t)1e-6) {
        ...
        scalar_t tau =
            sign / (lattice_abs(t) + lattice_sqrt((scalar_t)1 + t * t));
