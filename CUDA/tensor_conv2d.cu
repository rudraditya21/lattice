...
extern "C" __global__ void lattice_conv2d(const scalar_t* input,
    ...
    unsigned long long row =
        static_cast<unsigned long long>(blockIdx.y) * blockDim.y + threadIdx.y;
    ...
    for (unsigned long long kr = 0; kr < params.k_h; ++kr) {
        for (unsigned long long kc = 0; kc < params.k_w; ++kc) {
            ...
            acc +=
                input[in_r * params.in_w + in_c] * kernel[kr * params.k_w + kc];
        }
    }
