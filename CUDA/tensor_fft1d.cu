...
extern "C" __global__ void lattice_fft1d(const scalar_t* input,
                                         scalar_t* out_real,
                                         scalar_t* out_imag,
                                         lattice_fft_params_t params) {
    ...
    for (unsigned long long t = 0; t < params.n; ++t) {
        scalar_t angle = -two_pi * static_cast<scalar_t>(k * t) /
                         static_cast<scalar_t>(params.n);
