...
struct Type {
    ...
    // Optional element dtypes for tuples; empty means unknown length in dynamic
    // mode.
    ...
    static Type Record(
        std::vector<std::pair<std::string, std::optional<DType>>> fields) {
        Type t;
    ...
    static Type Tensor(TensorKind kind,
