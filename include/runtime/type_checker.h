...
// A lightweight static type checker that validates annotated
// parameters/returns/bindings, propagates basic expression types, and rejects
// implicit narrowing. Explicit narrowing is allowed via the cast builtin:
// cast(typeName, expr).
class TypeChecker {
   public:
    TypeChecker();
   ...
   private:
    ...
    bool IsAssignable(const std::optional<Type>& from,
