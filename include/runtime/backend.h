...
class StatusOr {
   public:
    ...
    StatusOr(T v)
        : status_(Status::OK()), value_(std::move(v)), has_value_(true) {}
    template <typename U,
              typename = std::enable_if_t<std::is_convertible<U, T>::value>>
    StatusOr(U&& v)
        : status_(Status::OK()), value_(std::forward<U>(v)), has_value_(true) {}
...
class Backend {
   public:
    ...
    virtual StatusOr<std::shared_ptr<Event>> CreateEvent() const = 0;
    virtual StatusOr<Allocation> Allocate(size_t bytes,
                                          size_t alignment = 64) const = 0;
    virtual Status Deallocate(const Allocation& alloc) const = 0;
    virtual StatusOr<Allocation> AllocatePinned(
        size_t bytes,
        size_t alignment = 64) const = 0;
