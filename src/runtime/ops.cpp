...
bool IsFloat(DType t) {
    return t == DType::kF16 || t == DType::kBF16 || t == DType::kF32 ||
           t == DType::kF64;
}
bool IsSignedInt(DType t) {
    return t == DType::kI8 || t == DType::kI16 || t == DType::kI32 ||
           t == DType::kI64;
}
bool IsUnsignedInt(DType t) {
    return t == DType::kU8 || t == DType::kU16 || t == DType::kU32 ||
           t == DType::kU64;
}
...
uint64_t Threefry2x32(uint64_t counter, uint64_t key) {
    ...
    auto rotl = [](uint32_t v, uint32_t r) {
        return (v << r) | (v >> (32 - r));
    };
    ...
    switch (v.type) {
        ...
        case DType::kC128:
            if (!finite(v.complex.real()) || !finite(v.complex.imag())) {
                throw util::Error("Non-finite complex result (NaN/inf)", line,
                                  column);
            }
            break;
        case DType::kTensor: {
            ...
            for (int64_t i = 0; i < v.tensor.size; ++i) {
                if (!finite(data[i])) {
                    throw util::Error(
                        "Tensor contains non-finite value (NaN/inf)", line,
                        column);
                }
            }
            break;
...
bool IsNumericTypeName(const std::string& name) {
    static const std::unordered_set<std::string> kNumeric = {
        ...
        "u32",       "u64",        "f16",     "f32",     "f64", "bfloat16",
        "complex64", "complex128", "decimal", "rational"};
...
bool ValueMatchesType(const Value& value, const std::string& type_name) {
    ...
    if (IsNumericTypeName(type_name)) {
        if (value.type == DType::kBool || value.type == DType::kFunction)
            return false;
        if (value.type_name.empty())
            return true;
        if (IsNumericTypeName(value.type_name))
            return true;
...
std::string CombineNumericType(const Value& lhs, const Value& rhs) {
    auto is_floatish = [](const std::string& t) {
        return t == "f16" || t == "f32" || t == "f64" || t == "bfloat16" ||
               t.rfind("complex", 0) == 0;
    };
    auto is_intish = [](const std::string& t) {
        return !t.empty() && (t[0] == 'i' || t[0] == 'u');
    };
...
std::optional<DType> LookupDType(const std::string& name) {
    ...
    if (name == "complex128")
        return DType::kC128;
    if (name == "decimal")
        return DType::kDecimal;
    if (name == "rational")
        return DType::kRational;
    return std::nullopt;
}
...
DType PromoteType(DType a, DType b, int line, int column) {
    ...
    if (a == DType::kDecimal && b == DType::kDecimal)
        return DType::kDecimal;
    if (a == DType::kRational && b == DType::kRational)
        return DType::kRational;
    if (is_decimal_or_rational(a) || is_decimal_or_rational(b)) {
        throw util::Error(
            "Decimal/rational cross-type arithmetic not supported in promotion",
            line, column);
    }
    ...
    if (signed_rank > 0)
        return RankToSigned(signed_rank);
    if (unsigned_rank > 0)
        return RankToUnsigned(unsigned_rank);
    return a;
}
...
Value CastTo(DType target, const Value& v, int line, int column) {
    auto as_double = [&]() -> double {
        switch (v.type) {
            ...
            case DType::kRational:
                return static_cast<double>(v.rational.num) /
                       static_cast<double>(v.rational.den);
            default:
                throw util::Error(
                    "Cannot cast complex/decimal/rational/string/function",
                    line, column);
        }
    };
    auto as_signed = [&]() -> int64_t {
        switch (v.type) {
            ...
            default:
                throw util::Error(
                    "Cannot cast complex/decimal/rational/function", line,
                    column);
        }
    };
        ...
        case DType::kC64:
            ...
            if (v.type == DType::kC128) {
                return Value::Complex64(
                    std::complex<float>(static_cast<float>(v.complex.real()),
                                        static_cast<float>(v.complex.imag())));
            }
            return Value::Complex64(
                std::complex<float>(static_cast<float>(as_double()), 0.0f));
        case DType::kC128:
            if (v.type == DType::kC128) {
                return v;
            }
...
std::string DeriveTypeName(const Value& v) {
    ...
    if (v.type == DType::kBool)
        return "bool";
    if (v.type == DType::kI32 || v.type == DType::kI64 ||
        v.type == DType::kU32 || v.type == DType::kU64) {
        return v.type_name;
    }
...
Value Evaluator::Evaluate(const parser::Expression& expr) {
    ...
    if (const auto* str_lit =
            dynamic_cast<const parser::StringLiteral*>(&expr)) {
        return Value::String(str_lit->value);
    }
    if (const auto* unary =
            dynamic_cast<const parser::UnaryExpression*>(&expr)) {
        return EvaluateUnary(*unary);
    }
    if (const auto* binary =
            dynamic_cast<const parser::BinaryExpression*>(&expr)) {
        return EvaluateBinary(*binary);
    }
    if (const auto* identifier =
            dynamic_cast<const parser::Identifier*>(&expr)) {
    ...
    if (const auto* record =
            dynamic_cast<const parser::RecordLiteral*>(&expr)) {
    ...
    if (const auto* idx = dynamic_cast<const parser::IndexExpression*>(&expr)) {
        ...
        if (object.type == DType::kTuple) {
            ...
            if (pos < 0 ||
                pos >= static_cast<int64_t>(object.tuple.elements.size())) {
                throw util::Error("Tuple index out of range", idx->line,
                                  idx->column);
            }
            return object.tuple.elements[static_cast<size_t>(pos)];
        } else if (object.type == DType::kRecord) {
            if (index.type != DType::kString) {
                throw util::Error("Record index must be a string", idx->line,
                                  idx->column);
            }
            auto it = object.record.index.find(index.str);
            if (it == object.record.index.end()) {
                throw util::Error("Record key not found: " + index.str,
                                  idx->line, idx->column);
            }
            return object.record.fields[it->second].second;
        }
        throw util::Error("Indexing not supported on this type", idx->line,
                          idx->column);
    }
    ...
    switch (expr.op) {
        case parser::UnaryOp::kNegate:
            switch (operand.type) {
                ...
                case DType::kU64:
                    return ensure(
                        Value::I64(-static_cast<int64_t>(operand.u64)));
                case DType::kF32:
                    return ensure(Value::F32(-static_cast<float>(operand.f64)));
                case DType::kF64:
                    return ensure(Value::F64(-operand.f64));
                default:
                    throw util::Error("Unary negate not supported for type " +
...
Value Evaluator::EvaluateBinary(const parser::BinaryExpression& expr) {
        ...
        switch (lhs.type) {
            case DType::kI32: {
                ...
                switch (expr.op) {
                    ...
                    case parser::BinaryOp::kDiv:
                        if (rv == 0) {
                            throw util::Error("Division by zero", expr.line,
                                              expr.column);
                        }
                        return ensure(Value::F64(static_cast<double>(lv) /
            ...
            case DType::kC128: {
                ...
                switch (expr.op) {
                    ...
                    case parser::BinaryOp::kLt:
                    case parser::BinaryOp::kLe:
                        throw util::Error("Complex comparison not supported",
            ...
            case DType::kString: {
                switch (expr.op) {
                    ...
                    default:
                        throw util::Error(
                            "Only equality/inequality supported for strings",
                            expr.line, expr.column);
                }
            } break;
            default:
    ...
    if (lhs.type == DType::kTensor || rhs.type == DType::kTensor) {
        if (expr.op == parser::BinaryOp::kAdd ||
            ...
            expr.op == parser::BinaryOp::kMul ||
            expr.op == parser::BinaryOp::kDiv) {
            std::string gpu_error;
            auto gpu_value = TryGpuElemwise(lhs, rhs, expr.op, expr.line,
                                            expr.column, &gpu_error);
        ...
        if ((lhs.type == DType::kTensor &&
            ...
            (rhs.type == DType::kTensor &&
             rhs.tensor.kind == TensorKind::kRagged)) {
            if (lhs.type != DType::kTensor || rhs.type != DType::kTensor ||
                lhs.tensor.kind != TensorKind::kRagged ||
                rhs.tensor.kind != TensorKind::kRagged) {
                throw util::Error(
                    "Ragged tensors only support ops with matching ragged "
                    "tensors (use to_dense)",
                    expr.line, expr.column);
            }
            if (lhs.tensor.row_splits != rhs.tensor.row_splits) {
                throw util::Error(
                    "Ragged tensors must share identical row_splits for "
                    "elementwise ops",
                    expr.line, expr.column);
            }
            if (lhs.tensor.ragged_values.size() !=
                rhs.tensor.ragged_values.size()) {
                throw util::Error("Ragged value buffers must match in length",
                                  expr.line, expr.column);
            }
            Value out = Value::TensorRagged(
                lhs.tensor.row_splits, {},
                PromoteType(lhs.tensor.elem_type, rhs.tensor.elem_type,
                            expr.line, expr.column));
            ...
            for (size_t i = 0; i < out.tensor.ragged_values.size(); ++i) {
                ...
                switch (expr.op) {
                    ...
                    case parser::BinaryOp::kDiv:
                        ...
                        break;
                    default:
                        throw util::Error("Ragged tensors only support +,-,*,/",
                                          expr.line, expr.column);
                }
            }
            return out;
        ...
        if ((lhs.type == DType::kTensor &&
            ...
            if (lhs.type == DType::kTensor && rhs.type == DType::kTensor &&
                ...
                if (lhs.tensor.shape != rhs.tensor.shape) {
                    throw util::Error(
                        "Sparse tensors must share shape for elementwise ops",
                        expr.line, expr.column);
                }
                ...
                DType elem_target = PromoteType(dense_l.tensor.elem_type,
                                                dense_r.tensor.elem_type,
                                                expr.line, expr.column);
                ...
                for (int64_t i = 0; i < out.tensor.size; ++i) {
                    ...
                    switch (expr.op) {
                        ...
                        default:
                            throw util::Error(
                                "Sparse tensor comparison not supported",
                                expr.line, expr.column);
                    }
                }
                return lhs.tensor.kind == TensorKind::kSparseCSR
                           ? DenseToCSR(out)
                           : DenseToCOO(out);
            }
            // Different sparse formats or sparse with ragged already handled
            // above: error if both are sparse but formats differ; else densify
            // the sparse side when mixed with dense.
            if (lhs.type == DType::kTensor && rhs.type == DType::kTensor &&
                lhs.tensor.kind != rhs.tensor.kind &&
                lhs.tensor.kind != TensorKind::kDense &&
                rhs.tensor.kind != TensorKind::kDense) {
                throw util::Error(
                    "Sparse tensor formats must match; convert first",
                    expr.line, expr.column);
            }
            Value lhs_d = lhs.type == DType::kTensor &&
                                  lhs.tensor.kind != TensorKind::kDense
                              ? ToDenseTensor(lhs, expr.line, expr.column)
                              : lhs;
            Value rhs_d = rhs.type == DType::kTensor &&
                                  rhs.tensor.kind != TensorKind::kDense
        ...
        const std::vector<int64_t> lhs_shape = lhs.type == DType::kTensor
                                                   ? lhs.tensor.shape
                                                   : std::vector<int64_t>{};
        const std::vector<int64_t> rhs_shape = rhs.type == DType::kTensor
                                                   ? rhs.tensor.shape
                                                   : std::vector<int64_t>{};
        auto broadcast_shape = BroadcastShape(lhs_shape, rhs_shape);
        if (!broadcast_shape.has_value()) {
            throw util::Error("Tensor shapes are not broadcastable (lhs " +
                                  ShapeToString(lhs_shape) + ", rhs " +
                                  ShapeToString(rhs_shape) + ")",
                              0, 0);
        }
        ...
        if (lhs.type == DType::kTensor && rhs.type == DType::kTensor) {
            ...
            elem_target = PromoteType(lhs.tensor.elem_type, rhs.type, expr.line,
                                      expr.column);
        } else {
            elem_target = PromoteType(rhs.tensor.elem_type, lhs.type, expr.line,
                                      expr.column);
        }
        Value out = Value::Tensor(out_shape, elem_target, 0.0);
        ...
        std::vector<int64_t> rhs_bstrides =
            rhs.type == DType::kTensor
                ? BroadcastStrides(rhs.tensor.shape, rhs.tensor.strides,
                                   out_shape.size())
                : std::vector<int64_t>(out_shape.size(), 0);
        ...
        auto lhs_data = [&](int64_t idx) -> double {
            ...
            const int64_t offset =
                OffsetFromFlatIndex(idx, out.tensor.strides, lhs_bstrides);
            return lhs.tensor.Data()[offset];
        };
        auto rhs_data = [&](int64_t idx) -> double {
            if (rhs.type != DType::kTensor)
                return rhs.f64;
            const int64_t offset =
                OffsetFromFlatIndex(idx, out.tensor.strides, rhs_bstrides);
    ...
    if (lhs.type == DType::kTuple && rhs.type == DType::kTuple) {
        if (expr.op == parser::BinaryOp::kEq ||
            expr.op == parser::BinaryOp::kNe) {
            bool equal = lhs.tuple.elements.size() == rhs.tuple.elements.size();
            if (equal) {
                for (size_t i = 0; i < lhs.tuple.elements.size(); ++i) {
                    if (lhs.tuple.elements[i].ToString() !=
                        rhs.tuple.elements[i].ToString()) {
                        equal = false;
                        break;
                    }
                }
            }
            return Value::Bool(expr.op == parser::BinaryOp::kEq ? equal
                                                                : !equal);
        }
        ...
        if (expr.op == parser::BinaryOp::kEq ||
            ...
            if (equal) {
                for (size_t i = 0; i < lhs.record.fields.size(); ++i) {
                    if (lhs.record.fields[i].first !=
                            rhs.record.fields[i].first ||
                        lhs.record.fields[i].second.ToString() !=
                            rhs.record.fields[i].second.ToString()) {
                        equal = false;
                        break;
                    }
                }
    ...
    auto is_int = [](DType t) {
        return t == DType::kI8 || t == DType::kI16 || t == DType::kI32 ||
               t == DType::kI64 || t == DType::kU8 || t == DType::kU16 ||
               t == DType::kU32 || t == DType::kU64;
    };
    auto is_float = [](DType t) {
        return t == DType::kF16 || t == DType::kBF16 || t == DType::kF32 ||
               t == DType::kF64;
    };
    auto is_complex = [](DType t) {
        return t == DType::kC64 || t == DType::kC128;
    };
    ...
    if (target == DType::kDecimal) {
        ...
        switch (expr.op) {
            ...
            case parser::BinaryOp::kDiv:
                if (rv == 0.0L)
                    throw util::Error("Division by zero", 0, 0);
                return Value::Decimal(RoundDecimal(lv / rv));
            case parser::BinaryOp::kEq:
    ...
    if (is_complex(target)) {
        std::complex<double> lcv =
            target == DType::kC128
                ? lcast.complex
                : std::complex<double>(
                      static_cast<double>(lcast.complex.real()),
                      static_cast<double>(lcast.complex.imag()));
        std::complex<double> rcv =
            target == DType::kC128
                ? rcast.complex
                : std::complex<double>(
                      static_cast<double>(rcast.complex.real()),
                      static_cast<double>(rcast.complex.imag()));
        switch (expr.op) {
            case parser::BinaryOp::kAdd: {
                auto res = lcv + rcv;
                return ensure(target == DType::kC64
                                  ? Value::Complex64(std::complex<float>(
                                        static_cast<float>(res.real()),
            ...
            case parser::BinaryOp::kLt:
            case parser::BinaryOp::kLe:
                throw util::Error("Complex comparison not supported", expr.line,
                                  expr.column);
        }
    }
        ...
        if (is_unsigned) {
            ...
            switch (expr.op) {
                case parser::BinaryOp::kAdd:
                    return ensure(
                        target == DType::kU32
                            ? Value::U32(static_cast<uint32_t>(lv + rv))
                            : Value::U64(lv + rv));
                case parser::BinaryOp::kSub:
                    return ensure(
                        target == DType::kU32
            ...
            switch (expr.op) {
                case parser::BinaryOp::kAdd:
                    switch (target) {
                        case DType::kI8:
                            return ensure(
                                Value::I8(static_cast<int8_t>(lv + rv)));
                        case DType::kI16:
                            return ensure(
                                Value::I16(static_cast<int16_t>(lv + rv)));
                        case DType::kI32:
                            return ensure(
                                Value::I32(static_cast<int32_t>(lv + rv)));
                        default:
                            return ensure(Value::I64(lv + rv));
                    }
                case parser::BinaryOp::kSub:
...
Value Evaluator::EvaluateIdentifier(const parser::Identifier& identifier) {
    if (!env_) {
        throw util::Error("Environment is not configured", identifier.line,
                          identifier.column);
    }
    auto value = env_->Get(identifier.name);
    if (!value.has_value()) {
        throw util::Error("Undefined identifier: " + identifier.name,
...
Value Evaluator::EvaluateCall(const parser::CallExpression& call) {
    ...
    if (call.callee == "cast") {
        if (call.args.size() != 2) {
            throw util::Error(
                "cast expects two arguments: type name and expression",
                call.line, call.column);
        }
        const auto* type_id =
            dynamic_cast<const parser::Identifier*>(call.args[0].get());
        if (type_id == nullptr) {
            throw util::Error(
                "cast first argument must be a type name identifier", call.line,
                call.column);
        }
        auto dt = LookupDType(type_id->name);
        if (!dt.has_value()) {
            throw util::Error("Unknown cast target type: " + type_id->name,
                              call.line, call.column);
        }
    ...
    if (found.has_value() && found->type == DType::kFunction) {
        ...
        if (fn == nullptr) {
            throw util::Error("Function is null: " + name, call.line,
                              call.column);
        }
        if (fn->body == nullptr) {
            if (name == "print") {
                if (args.size() != 1) {
                    throw util::Error("print expects 1 argument", call.line,
                                      call.column);
                }
                std::cout << args[0].ToString() << "\n";
                return Value::Number(0.0);
            }
            throw util::Error("Function has no body: " + name, call.line,
                              call.column);
        }
        if (fn->parameters.size() != args.size()) {
            throw util::Error(name + " expects " +
        ...
        for (size_t i = 0; i < args.size(); ++i) {
            if (!fn->parameter_types.empty() &&
                i < fn->parameter_types.size()) {
                const std::string& annot = fn->parameter_types[i];
                if (!annot.empty() && !ValueMatchesType(args[i], annot)) {
                    throw util::Error("Type mismatch for parameter '" +
                                          fn->parameters[i] + "' (expected " +
                                          annot + ", got " +
                                          ValueTypeName(args[i]) + ")",
                                      call.line, call.column);
                }
            }
        ...
        if (!fn->return_type.empty()) {
            if (!body_result.value.has_value()) {
                throw util::Error("Missing return value in function " + name,
                                  call.line, call.column);
            }
        ...
        if (body_result.control == ControlSignal::kReturn &&
            body_result.value.has_value()) {
    ...
    auto expect_args = [&](size_t count, const std::string& func) {
        if (args.size() != count) {
            throw util::Error(
                func + " expects " + std::to_string(count) + " arguments",
                call_line, call_col);
        }
    };
    auto ensure_not_tensor = [&](const Value& v, const std::string& func) {
        if (v.type == DType::kTensor) {
            throw util::Error(func + " does not support tensor arguments",
                              call_line, call_col);
        }
    };
    ...
    if (name == "rational") {
        ...
        if (den == 0) {
            throw util::Error("rational denominator cannot be zero", call_line,
                              call_col);
        }
        return Value::RationalValueNormalized(num, den);
    }
    ...
    if (name == "tensor") {
        if (args.size() < 2) {
            throw util::Error("tensor expects at least shape and fill value",
                              call_line, call_col);
        }
        ...
        if (shape.empty()) {
            throw util::Error("tensor shape cannot be empty", call_line,
                              call_col);
        }
        for (int64_t dim : shape) {
            if (dim <= 0) {
                throw util::Error("tensor dimensions must be positive (got " +
                                      std::to_string(dim) + ")",
                                  call_line, call_col);
            }
        }
    ...
    if (name == "tensor_values") {
        if (args.empty()) {
            throw util::Error("tensor_values expects at least one value",
                              call_line, call_col);
        }
        for (const auto& v : args) {
            if (v.type == DType::kTensor) {
                throw util::Error(
                    "tensor_values does not accept tensor arguments", call_line,
                    call_col);
            }
        }
        // Support nested tuple/list literals to build multi-dimensional
        // tensors.
        struct FlattenState {
            std::vector<int64_t> shape;
        ...
        std::function<void(const Value&, size_t)> flatten = [&](const Value& v,
                                                                size_t depth) {
            if (v.type == DType::kTuple) {
                ...
                if (depth == st.shape.size()) {
                    ...
                    throw util::Error(
                        "tensor_values nested lists must have consistent "
                        "lengths",
                        call_line, call_col);
                }
            ...
            if (!st.has_elem) {
                ...
                st.elem_type =
                    PromoteType(st.elem_type, v.type, call_line, call_col);
            }
        };
        ...
        if (args.size() == 1 && args[0].type == DType::kTuple) {
            ...
            for (const auto& v : args)
                flatten(v, 1);  // depth offset to keep shape size=1
        }

        if (!st.has_elem || st.leaves.empty()) {
            throw util::Error("tensor_values needs at least one scalar",
                              call_line, call_col);
        }
        int64_t total = 1;
        for (int64_t d : st.shape) {
            if (d <= 0) {
                throw util::Error("tensor dimensions must be positive",
                                  call_line, call_col);
            }
            total *= d;
        }
        if (static_cast<int64_t>(st.leaves.size()) != total) {
            throw util::Error(
                "tensor_values data does not match inferred shape", call_line,
                call_col);
        }
    ...
    if (name == "tensor_sparse_csr") {
        if (args.size() != 4) {
            throw util::Error(
                "tensor_sparse_csr expects (shape_tuple, indptr_tuple, "
                "indices_tuple, values_tuple)",
                call_line, call_col);
        }
        auto extract_tuple_i64 = [&](const Value& v, const std::string& what) {
            if (v.type != DType::kTuple) {
                throw util::Error(what + " must be a tuple", call_line,
                                  call_col);
            }
        ...
        if (shape.size() != 2) {
            throw util::Error("tensor_sparse_csr shape must be length 2",
                              call_line, call_col);
        }
        ...
        if (indptr.size() != static_cast<size_t>(shape[0] + 1)) {
            throw util::Error("indptr length must be rows+1", call_line,
                              call_col);
        }
        ...
        if (indices.size() != vals_tuple.tuple.elements.size()) {
            throw util::Error("indices and values must have same length",
                              call_line, call_col);
        }
        ...
        return Value::TensorSparseCSR(std::move(shape), std::move(indptr),
                                      std::move(indices), std::move(vals),
                                      elem_type);
    }
    if (name == "tensor_sparse_coo") {
        if (args.size() != 4) {
            throw util::Error(
                "tensor_sparse_coo expects (shape_tuple, rows_tuple, "
                "cols_tuple, values_tuple)",
                call_line, call_col);
        }
        auto extract_tuple_i64 = [&](const Value& v, const std::string& what) {
            if (v.type != DType::kTuple) {
        ...
        auto shape = extract_tuple_i64(args[0], "shape");
        if (shape.size() != 2) {
            throw util::Error("tensor_sparse_coo shape must be length 2",
                              call_line, call_col);
        }
        ...
        if (rows_v.size() != cols_v.size() ||
            rows_v.size() != vals_tuple.tuple.elements.size()) {
            throw util::Error("rows, cols, and values must have same length",
                              call_line, call_col);
        }
        std::vector<double> vals;
        vals.reserve(vals_tuple.tuple.elements.size());
        ...
        return Value::TensorSparseCOO(std::move(shape), std::move(rows_v),
                                      std::move(cols_v), std::move(vals),
                                      elem_type);
    }
    if (name == "tensor_ragged") {
        if (args.size() != 2) {
            throw util::Error(
                "tensor_ragged expects (row_splits_tuple, values_tuple)",
                call_line, call_col);
        }
        auto extract_tuple_i64 = [&](const Value& v, const std::string& what) {
            if (v.type != DType::kTuple) {
        ...
        if (splits.size() < 2) {
            throw util::Error("row_splits must have at least 2 entries",
                              call_line, call_col);
        }
        for (size_t i = 1; i < splits.size(); ++i) {
            if (splits[i] < splits[i - 1]) {
                throw util::Error("row_splits must be non-decreasing",
                                  call_line, call_col);
            }
        }
        ...
        return Value::TensorRagged(std::move(splits), std::move(vals),
                                   elem_type);
    }
    if (name == "to_dense") {
        expect_args(1, name);
        ...
        throw util::Error("to_dense for ragged tensors is not supported yet",
                          call_line, call_col);
    }
    if (name == "to_sparse_csr") {
        ...
        const Value& v = args[0];
        if (!IsDenseTensor(v)) {
            throw util::Error("to_sparse_csr expects a dense tensor", call_line,
                              call_col);
        }
        if (v.tensor.shape.size() != 2) {
            throw util::Error("to_sparse_csr supports only 2D tensors",
                              call_line, call_col);
        }
        ...
        return Value::TensorSparseCSR({rows, cols}, std::move(indptr),
                                      std::move(indices), std::move(vals),
                                      v.tensor.elem_type);
    }
    if (name == "to_sparse_coo") {
        ...
        const Value& v = args[0];
        if (!IsDenseTensor(v)) {
            throw util::Error("to_sparse_coo expects a dense tensor", call_line,
                              call_col);
        }
        if (v.tensor.shape.size() != 2) {
            throw util::Error("to_sparse_coo supports only 2D tensors",
                              call_line, call_col);
        }
        int64_t rows = v.tensor.shape[0];
        int64_t cols = v.tensor.shape[1];
        ...
        return Value::TensorSparseCOO({rows, cols}, std::move(r), std::move(c),
    ...
    if (name == "pow") {
        ...
        if ((args[0].type == DType::kC64 || args[0].type == DType::kC128) ||
            ...
            std::complex<double> base =
                args[0].type == DType::kC128
                    ? args[0].complex
                    : std::complex<double>(args[0].f64, 0.0);
            std::complex<double> expv =
                args[1].type == DType::kC128
                    ? args[1].complex
                    : std::complex<double>(args[1].f64, 0.0);
    ...
    if (name == "gcd") {
        ...
        ensure_not_tensor(args[0], name);
        ensure_not_tensor(args[1], name);
        if (IsComplex(args[0].type) || IsComplex(args[1].type) ||
            IsFloat(args[0].type) || IsFloat(args[1].type) ||
            args[0].type == DType::kDecimal ||
            args[0].type == DType::kRational ||
            args[1].type == DType::kDecimal ||
            args[1].type == DType::kRational) {
            throw util::Error("gcd requires integer arguments", call_line,
                              call_col);
        }
        auto a = static_cast<long long>(
            CastTo(DType::kI64, args[0], call_line, call_col).i64);
        auto b = static_cast<long long>(
            CastTo(DType::kI64, args[1], call_line, call_col).i64);
    ...
    if (name == "lcm") {
        ...
        if (IsComplex(args[0].type) || IsComplex(args[1].type) ||
            ...
            args[1].type == DType::kDecimal ||
            args[1].type == DType::kRational) {
            throw util::Error("lcm requires integer arguments", call_line,
                              call_col);
        }
        auto a = static_cast<long long>(
            CastTo(DType::kI64, args[0], call_line, call_col).i64);
    ...
    if (name == "abs") {
        ...
        switch (args[0].type) {
            ...
            case DType::kRational:
                return Value::RationalValueNormalized(
                    std::abs(args[0].rational.num),
    ...
    if (name == "mod") {
        ...
        ensure_not_tensor(args[1], name);
        if (args[1].f64 == 0.0) {
            throw util::Error("mod divisor cannot be zero", call_line,
                              call_col);
        }
        if (!IsFloat(args[0].type) && !IsFloat(args[1].type) &&
            !IsComplex(args[0].type) && !IsComplex(args[1].type) &&
            args[0].type != DType::kDecimal &&
            args[0].type != DType::kRational &&
            args[1].type != DType::kDecimal &&
            args[1].type != DType::kRational) {
    ...
    if (name == "sum") {
        ...
        if (v.tensor.kind != TensorKind::kRagged) {
            std::string gpu_error;
            auto gpu_value = TryGpuReduce(v, ReduceKind::kSum, call_line,
                                          call_col, &gpu_error);
            if (gpu_value.has_value())
                return gpu_value.value();
        }
        auto finish_as_elem = [&](double total) {
            return CastTo(v.tensor.elem_type, Value::F64(total), call_line,
                          call_col);
        };
        if (v.tensor.kind == TensorKind::kDense) {
            double total = 0.0;
        ...
        if (v.tensor.kind == TensorKind::kSparseCSR ||
            v.tensor.kind == TensorKind::kSparseCOO) {
            double total = 0.0;
            for (double val : v.tensor.sparse_values)
                total += val;
            return finish_as_elem(total);
        }
        if (v.tensor.kind == TensorKind::kRagged) {
            double total = 0.0;
            for (double val : v.tensor.ragged_values)
                total += val;
            return finish_as_elem(total);
        }
        throw util::Error("Unsupported tensor kind for sum", call_line,
                          call_col);
    }
    if (name == "mean") {
        expect_args(1, name);
        ...
        if (v.tensor.kind != TensorKind::kRagged) {
            std::string gpu_error;
            auto gpu_value = TryGpuReduce(v, ReduceKind::kMean, call_line,
                                          call_col, &gpu_error);
            if (gpu_value.has_value())
                return gpu_value.value();
        }
        auto finish_as_elem = [&](double mean) {
            return CastTo(v.tensor.elem_type, Value::F64(mean), call_line,
                          call_col);
        };
        if (v.tensor.kind == TensorKind::kDense) {
            double total = 0.0;
        ...
        if (v.tensor.kind == TensorKind::kSparseCSR ||
            ...
            for (double val : v.tensor.sparse_values)
                total += val;
            double denom =
                static_cast<double>(v.tensor.shape[0] * v.tensor.shape[1]);
        ...
        throw util::Error("Unsupported tensor kind for mean", call_line,
                          call_col);
    }
    if (name == "var" || name == "std") {
        ...
        if (v.tensor.kind != TensorKind::kRagged) {
            std::string gpu_error;
            auto gpu_value = TryGpuReduce(
                v, name == "var" ? ReduceKind::kVar : ReduceKind::kStd,
                call_line, call_col, &gpu_error);
            if (gpu_value.has_value())
                return gpu_value.value();
        }
        auto finish_as_elem = [&](double x) {
            return CastTo(v.tensor.elem_type, Value::F64(x), call_line,
                          call_col);
        };
        auto compute = [&](auto accessor, double count) -> Value {
            ...
            if (name == "var")
                return finish_as_elem(variance);
            return finish_as_elem(std::sqrt(variance));
        };
        if (v.tensor.kind == TensorKind::kDense) {
            return compute(
                [&]() {
                    return std::vector<double>(v.tensor.Data(),
                                               v.tensor.Data() + v.tensor.size);
                },
        ...
        if (v.tensor.kind == TensorKind::kSparseCSR ||
            ...
            std::vector<double> dense_vals(
                static_cast<size_t>(v.tensor.shape[0] * v.tensor.shape[1]),
                0.0);
            if (v.tensor.kind == TensorKind::kSparseCSR) {
                for (size_t row = 0; row + 1 < v.tensor.indptr.size(); ++row) {
                    int64_t start = v.tensor.indptr[row];
                    ...
                    for (int64_t idx = start; idx < end; ++idx) {
                        int64_t col = v.tensor.indices[idx];
                        dense_vals[static_cast<size_t>(
                            row_i * v.tensor.shape[1] + col)] =
                ...
                for (size_t i = 0; i < v.tensor.rows.size(); ++i) {
                    int64_t row = v.tensor.rows[i];
                    int64_t col = v.tensor.cols[i];
                    dense_vals[static_cast<size_t>(row * v.tensor.shape[1] +
                                                   col)] =
        ...
        throw util::Error("Unsupported tensor kind for variance/std", call_line,
                          call_col);
    }
    if (name == "len") {
        expect_args(1, name);
        ...
        throw util::Error("len expects tuple, record, or tensor", call_line,
                          call_col);
    }
    if (name == "transpose") {
        expect_args(1, name);
        const Value& v = args[0];
        if (v.type != DType::kTensor) {
            throw util::Error("transpose expects a tensor", call_line,
                              call_col);
        }
        Value dense = ToDenseTensor(v, call_line, call_col);
        if (dense.tensor.shape.size() != 2) {
            throw util::Error("transpose supports only 2D tensors", call_line,
                              call_col);
        }
        {
            std::string gpu_error;
            auto gpu_value =
                TryGpuTranspose(dense, call_line, call_col, &gpu_error);
            if (gpu_value.has_value())
                return gpu_value.value();
        }
        int64_t rows = dense.tensor.shape[0];
        int64_t cols = dense.tensor.shape[1];
        Value out = Value::Tensor(std::vector<int64_t>{cols, rows},
                                  dense.tensor.elem_type, 0.0);
        for (int64_t r = 0; r < rows; ++r) {
            for (int64_t c = 0; c < cols; ++c) {
                out.tensor.Data()[c * rows + r] =
                    dense.tensor.Data()[r * cols + c];
            }
        }
        return out;
    }
    if (name == "matmul") {
        ...
        Value rhs = args[1];
        if (lhs.type != DType::kTensor || rhs.type != DType::kTensor) {
            throw util::Error("matmul expects tensor arguments", call_line,
                              call_col);
        }
        ...
        if (lhs.tensor.shape.size() != 2 || rhs.tensor.shape.size() != 2) {
            throw util::Error("matmul supports only 2D tensors", call_line,
                              call_col);
        }
    ...
    if (name == "conv2d") {
        ...
        if (input.type != DType::kTensor || kernel.type != DType::kTensor) {
            throw util::Error("conv2d expects tensor arguments", call_line,
                              call_col);
        }
        if (input.tensor.shape.size() != 2 || kernel.tensor.shape.size() != 2) {
            throw util::Error("conv2d supports 2D tensors only", call_line,
                              call_col);
        }
        ...
        if (kh > h || kw > w) {
            throw util::Error("conv2d kernel larger than input", call_line,
                              call_col);
        }
        {
            std::string gpu_error;
            auto gpu_value =
                TryGpuConv2d(input, kernel, call_line, call_col, &gpu_error);
            if (gpu_value.has_value())
                return gpu_value.value();
        }
        int64_t oh = h - kh + 1;
        int64_t ow = w - kw + 1;
        DType elem =
            PromoteTensorElem(input.tensor.elem_type, kernel.tensor.elem_type,
                              call_line, call_col);
            ...
            for (int64_t j = 0; j < ow; ++j) {
                ...
                for (int64_t ki = 0; ki < kh; ++ki) {
                    for (int64_t kj = 0; kj < kw; ++kj) {
                        acc += input.tensor.Data()[(i + ki) * w + (j + kj)] *
                               kernel.tensor.Data()[ki * kw + kj];
                    }
                }
    ...
    if (name == "max_pool2d") {
        ...
        int64_t ow = w / kw;
        {
            std::string gpu_error;
            auto gpu_value =
                TryGpuMaxPool2d(input, kh, kw, call_line, call_col, &gpu_error);
            if (gpu_value.has_value())
                return gpu_value.value();
        }
    ...
    if (name == "fft1d") {
        ...
        }
        {
            std::string gpu_error;
            auto gpu_value =
                TryGpuFft1d(input, call_line, call_col, &gpu_error);
            if (gpu_value.has_value())
                return gpu_value.value();
        }
    ...
    if (name == "solve") {
        ...
        if (A.type != DType::kTensor || B.type != DType::kTensor) {
            throw util::Error("solve expects tensor arguments", call_line,
                              call_col);
        }
        if (A.tensor.shape.size() != 2) {
            throw util::Error("solve expects a 2D coefficient matrix",
                              call_line, call_col);
        }
        int64_t n = A.tensor.shape[0];
        if (A.tensor.shape[1] != n) {
            throw util::Error("solve requires a square matrix", call_line,
                              call_col);
        }
        if (B.tensor.shape.size() == 1) {
            if (B.tensor.shape[0] != n) {
                throw util::Error("solve rhs length must match matrix rows",
                                  call_line, call_col);
            }
        } else if (B.tensor.shape.size() == 2) {
            if (B.tensor.shape[0] != n) {
                throw util::Error("solve rhs rows must match matrix rows",
                                  call_line, call_col);
            }
        } else {
            throw util::Error("solve rhs must be 1D or 2D tensor", call_line,
                              call_col);
        }
        {
            std::string gpu_error;
        ...
        DType elem = PromoteTensorElem(A.tensor.elem_type, B.tensor.elem_type,
                                       call_line, call_col);
        ...
        for (int64_t i = 0; i < n; ++i) {
            for (int64_t j = 0; j < n; ++j) {
                a[static_cast<size_t>(i)][static_cast<size_t>(j)] =
                    CastTo(
                        DType::kF64,
                        Value::F64(A.tensor.Data()[i * A.tensor.shape[1] + j]),
                        call_line, call_col)
                        .f64;
            }
        }
        ...
        std::vector<std::vector<double>> b(n,
                                           std::vector<double>(rhs_cols, 0.0));
        for (int64_t i = 0; i < n; ++i) {
            for (int64_t j = 0; j < rhs_cols; ++j) {
                int64_t idx =
                    B.tensor.shape.size() == 1 ? i : (i * rhs_cols + j);
                b[static_cast<size_t>(i)][static_cast<size_t>(j)] =
                    CastTo(DType::kF64, Value::F64(B.tensor.Data()[idx]),
                           call_line, call_col)
                        .f64;
            }
        }
        for (int64_t k = 0; k < n; ++k) {
            int64_t pivot = k;
            double maxv =
                std::abs(a[static_cast<size_t>(k)][static_cast<size_t>(k)]);
            for (int64_t i = k + 1; i < n; ++i) {
                double v =
                    std::abs(a[static_cast<size_t>(i)][static_cast<size_t>(k)]);
                if (v > maxv) {
                    maxv = v;
                    pivot = i;
                }
            ...
            if (pivot != k) {
                std::swap(a[static_cast<size_t>(pivot)],
                          a[static_cast<size_t>(k)]);
                std::swap(b[static_cast<size_t>(pivot)],
            ...
            for (int64_t i = 0; i < n; ++i) {
                ...
                for (int64_t j = k; j < n; ++j) {
                    a[static_cast<size_t>(i)][static_cast<size_t>(j)] -=
                        factor *
                        a[static_cast<size_t>(k)][static_cast<size_t>(j)];
                }
                for (int64_t j = 0; j < rhs_cols; ++j) {
                    b[static_cast<size_t>(i)][static_cast<size_t>(j)] -=
                        factor *
        ...
        for (int64_t i = 0; i < n * rhs_cols; ++i)
            out.tensor.Data()[i] = flat[static_cast<size_t>(i)];
        if (rhs_cols == 1) {
            out.tensor.shape = {n};
    ...
    auto make_dense_tensor = [&](const std::vector<double>& data,
                                 const std::vector<int64_t>& shape,
                                 DType elem) {
        Value t = Value::Tensor(shape, elem, 0.0);
        for (size_t i = 0; i < data.size(); ++i)
            t.tensor.Data()[static_cast<int64_t>(i)] = data[i];
        return t;
    };
    if (name == "lu") {
        expect_args(1, name);
        ...
        if (A.type != DType::kTensor || A.tensor.shape.size() != 2 ||
            ...
            throw util::Error("lu expects a square 2D tensor", call_line,
                              call_col);
        }
        {
            std::string gpu_error;
        ...
        for (int64_t i = 0; i < n; ++i)
            L[static_cast<size_t>(i * n + i)] = 1.0;
        for (int64_t k = 0; k < n; ++k) {
            for (int64_t j = k; j < n; ++j) {
                double sum = 0.0;
                for (int64_t s = 0; s < k; ++s)
                    sum += L[static_cast<size_t>(k * n + s)] *
                           U[static_cast<size_t>(s * n + j)];
                U[static_cast<size_t>(k * n + j)] =
                    A.tensor.Data()[k * A.tensor.shape[1] + j] - sum;
            }
            ...
            for (int64_t i = k + 1; i < n; ++i) {
                double sum = 0.0;
                for (int64_t s = 0; s < k; ++s)
                    sum += L[static_cast<size_t>(i * n + s)] *
                           U[static_cast<size_t>(s * n + k)];
                L[static_cast<size_t>(i * n + k)] =
                    (A.tensor.Data()[i * A.tensor.shape[1] + k] - sum) /
                    U[static_cast<size_t>(k * n + k)];
            }
        }
    ...
    if (name == "qr") {
        ...
        auto a_val = [&](int64_t i, int64_t j) {
            return A.tensor.Data()[i * n + j];
        };
        for (int64_t j = 0; j < n; ++j) {
            for (int64_t i = 0; i < m; ++i)
            ...
            norm = std::sqrt(norm);
            if (norm == 0.0)
                throw util::Error("qr rank deficiency", call_line, call_col);
            r[static_cast<size_t>(j)][static_cast<size_t>(j)] = norm;
            for (int64_t i = 0; i < m; ++i)
                q[static_cast<size_t>(i)][static_cast<size_t>(j)] /= norm;
        }
        std::vector<double> q_flat(static_cast<size_t>(m * n), 0.0);
        std::vector<double> r_flat(static_cast<size_t>(n * n), 0.0);
        for (int64_t i = 0; i < m; ++i)
            for (int64_t j = 0; j < n; ++j)
                q_flat[static_cast<size_t>(i * n + j)] =
                    q[static_cast<size_t>(i)][static_cast<size_t>(j)];
        for (int64_t i = 0; i < n; ++i)
            for (int64_t j = 0; j < n; ++j)
                r_flat[static_cast<size_t>(i * n + j)] =
    ...
    if (name == "svd") {
        ...
        int64_t n = A.tensor.shape[1];
        if (m != 2 || n != 2) {
            throw util::Error("svd currently supports 2x2 matrices only",
                              call_line, call_col);
        }
        {
            std::string gpu_error;
        ...
        double a = A.tensor.Data()[0], b = A.tensor.Data()[1],
               c = A.tensor.Data()[2], d = A.tensor.Data()[3];
        double s1s1 = a * a + c * c;
        double s2s2 = b * b + d * d;
        double off = a * b + c * d;
        double tr = s1s1 + s2s2;
    ...
    if (name == "igamma") {  // Regularized lower incomplete gamma P(a,x)
        ...
        if (a <= 0 || x < 0) {
            throw util::Error("igamma expects a>0 and x>=0", call_line,
                              call_col);
        }
        ...
        auto contfrac = [&](double aa, double xx) {
            ...
            for (int i = 1; i < 1000; ++i) {
                double an =
                    -static_cast<double>(i) * (static_cast<double>(i) - aa);
    ...
    if (name == "philox") {
        ...
        uint64_t seed = static_cast<uint64_t>(
            CastTo(DType::kU64, args[0], call_line, call_col).u64);
        uint64_t stream = static_cast<uint64_t>(
            CastTo(DType::kU64, args[1], call_line, call_col).u64);
        uint64_t ctr = static_cast<uint64_t>(
            CastTo(DType::kU64, args[2], call_line, call_col).u64);
    ...
    if (name == "normal_pdf") {
        ...
        if (sigma <= 0.0)
            throw util::Error("normal_pdf sigma must be > 0", call_line,
                              call_col);
        double z = (x - mu) / sigma;
    ...
    if (name == "normal_cdf") {
        ...
        double sigma = ScalarF64(args[2], call_line, call_col);
        if (sigma <= 0.0)
            throw util::Error("normal_cdf sigma must be > 0", call_line,
                              call_col);
    ...
    if (name == "normal_sample") {
        ...
        if (sigma <= 0.0)
            throw util::Error("normal_sample sigma must be > 0", call_line,
                              call_col);
        uint64_t seed = static_cast<uint64_t>(
            CastTo(DType::kU64, args[2], call_line, call_col).u64);
        uint64_t stream = static_cast<uint64_t>(
            CastTo(DType::kU64, args[3], call_line, call_col).u64);
        uint64_t ctr = static_cast<uint64_t>(
            CastTo(DType::kU64, args[4], call_line, call_col).u64);
        uint64_t key = seed ^ (stream + 0x9E3779B97F4A7C15ULL);
    ...
    if (name == "uniform_pdf") {
        ...
        if (!(a < b))
            throw util::Error("uniform_pdf requires a < b", call_line,
                              call_col);
        if (x < a || x > b)
            return Value::F64(0.0);
    ...
    if (name == "uniform_cdf") {
        ...
        double b = ScalarF64(args[2], call_line, call_col);
        if (!(a < b))
            throw util::Error("uniform_cdf requires a < b", call_line,
                              call_col);
        if (x <= a)
            return Value::F64(0.0);
        if (x >= b)
    ...
    if (name == "uniform_sample") {
        ...
        double b = ScalarF64(args[1], call_line, call_col);
        if (!(a < b))
            throw util::Error("uniform_sample requires a < b", call_line,
                              call_col);
        uint64_t seed = static_cast<uint64_t>(
            CastTo(DType::kU64, args[2], call_line, call_col).u64);
        uint64_t ctr = static_cast<uint64_t>(
            CastTo(DType::kU64, args[3], call_line, call_col).u64);
        double u =
            Uint64ToUnitDouble(Philox2x32(ctr, seed ^ 0x9E3779B97F4A7C15ULL));
    ...
    if (name == "exponential_pdf") {
        ...
        if (lambda <= 0)
            throw util::Error("exponential_pdf lambda must be > 0", call_line,
                              call_col);
        if (x < 0)
            throw util::Error("exponential_pdf domain requires x>=0", call_line,
                              call_col);
    ...
    if (name == "exponential_cdf") {
        ...
        double lambda = ScalarF64(args[1], call_line, call_col);
        if (lambda <= 0)
            throw util::Error("exponential_cdf lambda must be > 0", call_line,
                              call_col);
        if (x < 0)
            throw util::Error("exponential_cdf domain requires x>=0", call_line,
                              call_col);
    ...
    if (name == "exponential_sample") {
        ...
        if (lambda <= 0)
            throw util::Error("exponential_sample lambda must be > 0",
                              call_line, call_col);
        uint64_t seed = static_cast<uint64_t>(
            CastTo(DType::kU64, args[1], call_line, call_col).u64);
        uint64_t ctr = static_cast<uint64_t>(
            CastTo(DType::kU64, args[2], call_line, call_col).u64);
        double u = std::max(
            Uint64ToUnitDouble(Philox2x32(ctr, seed ^ 0x517cc1b727220a95ULL)),
            1e-12);
    ...
    if (name == "poisson_pmf") {
        ...
        double lambda = ScalarF64(args[1], call_line, call_col);
        if (lambda <= 0)
            throw util::Error("poisson_pmf lambda must be > 0", call_line,
                              call_col);
        if (k < 0 || std::floor(k) != k) {
            throw util::Error("poisson_pmf k must be non-negative integer",
                              call_line, call_col);
        }
        double pmf =
            std::exp(-lambda + k * std::log(lambda) - std::lgamma(k + 1.0));
    ...
    if (name == "poisson_sample") {
        ...
        double lambda = ScalarF64(args[0], call_line, call_col);
        if (lambda <= 0)
            throw util::Error("poisson_sample lambda must be > 0", call_line,
                              call_col);
        uint64_t seed = static_cast<uint64_t>(
            CastTo(DType::kU64, args[1], call_line, call_col).u64);
        uint64_t ctr = static_cast<uint64_t>(
    ...
    if (name == "binomial_pmf") {
        ...
        if (p < 0 || p > 1)
            throw util::Error("binomial_pmf p must be in [0,1]", call_line,
                              call_col);
        if (n < 0 || std::floor(n) != n)
            throw util::Error("binomial_pmf n must be integer >=0", call_line,
                              call_col);
        if (k < 0 || k > n || std::floor(k) != k) {
            throw util::Error("binomial_pmf k must be integer in [0,n]",
                              call_line, call_col);
        }
        double log_c = std::lgamma(n + 1.0) - std::lgamma(k + 1.0) -
                       std::lgamma(n - k + 1.0);
        double pmf =
            std::exp(log_c + k * std::log(p) + (n - k) * std::log(1 - p));
        return Value::F64(pmf);
    }
    if (name == "binomial_sample") {
        ...
        if (n < 0 || std::floor(n) != n)
            throw util::Error("binomial_sample n must be integer >=0",
                              call_line, call_col);
        if (p < 0 || p > 1)
            throw util::Error("binomial_sample p must be in [0,1]", call_line,
                              call_col);
        uint64_t seed = static_cast<uint64_t>(
            CastTo(DType::kU64, args[2], call_line, call_col).u64);
        uint64_t ctr = static_cast<uint64_t>(
    ...
    if (name == "quantile") {
        ...
        if (data.empty())
            throw util::Error("quantile of empty data", call_line, call_col);
        if (q < 0.0 || q > 1.0)
            throw util::Error("quantile q must be in [0,1]", call_line,
                              call_col);
        {
            std::string gpu_error;
            auto gpu_value =
                TryGpuQuantile(args[0], q, call_line, call_col, &gpu_error);
            if (gpu_value.has_value())
                return gpu_value.value();
        }
        ...
        double val = data[idx];
        if (idx + 1 < data.size())
            val = val + frac * (data[idx + 1] - data[idx]);
    ...
    if (name == "correlation") {
        ...
        if (x.size() != y.size() || x.empty()) {
            throw util::Error("correlation requires equal non-empty vectors",
                              call_line, call_col);
        }
        {
            std::string gpu_error;
            auto gpu_value = TryGpuCorrelation(args[0], args[1], call_line,
                                               call_col, &gpu_error);
            if (gpu_value.has_value())
                return gpu_value.value();
        }
        ...
        if (den == 0.0)
            throw util::Error("correlation undefined (zero variance)",
                              call_line, call_col);
    ...
    if (name == "regression") {
        ...
        auto y = Tensor1DToVector(args[1], call_line, call_col);
        if (x.size() != y.size() || x.empty()) {
            throw util::Error("regression requires equal non-empty vectors",
                              call_line, call_col);
        }
        {
            std::string gpu_error;
            auto gpu_value = TryGpuRegression(args[0], args[1], call_line,
                                              call_col, &gpu_error);
            if (gpu_value.has_value())
                return gpu_value.value();
        }
        ...
        if (denom == 0.0)
            throw util::Error("regression undefined (zero variance)", call_line,
                              call_col);
    ...
    if (name == "has_key") {
        ...
        if (v.type != DType::kRecord || key.type != DType::kString) {
            throw util::Error("has_key expects (record, string)", call_line,
                              call_col);
        }
        return Value::Bool(v.record.index.find(key.str) !=
                           v.record.index.end());
    }
    if (name == "floor") {
        expect_args(1, name);
    ...
    if (name == "clamp") {
        ...
        DType target = PromoteType(
            PromoteType(args[0].type, args[1].type, call_line, call_col),
        ...
        if (IsFloat(target)) {
            ...
            return target == DType::kF32 ? Value::F32(static_cast<float>(res))
                                         : Value::F64(res);
        }
...
ExecResult Evaluator::EvaluateWhile(const parser::WhileStatement& stmt) {
    ...
    while (true) {
        Value condition = Evaluate(*stmt.condition);
        if (!RequireBool(condition, stmt.condition->line,
...
ExecResult Evaluator::EvaluateFor(const parser::ForStatement& stmt) {
    ...
    while (true) {
        if (stmt.condition) {
            ...
            if (!RequireBool(cond_val, stmt.condition->line,
                             stmt.condition->column)) {
                break;
            }
        }
...
ExecResult Evaluator::EvaluateAssignment(
    const parser::AssignmentStatement& stmt) {
    if (!env_) {
        throw util::Error("Environment is not configured", 0, 0);
    }
    ...
    if (stmt.tuple_pattern) {
        if (stmt.annotation.type) {
            throw util::Error(
                "Annotations are not supported on destructuring assignments",
                stmt.line, stmt.column);
        }
        Value rhs = Evaluate(*stmt.value);
        if (rhs.type != DType::kTuple) {
            throw util::Error("Destructuring expects a tuple value",
                              stmt.tuple_pattern->line,
                              stmt.tuple_pattern->column);
        }
        if (rhs.tuple.elements.size() != stmt.tuple_pattern->names.size()) {
            throw util::Error("Tuple destructuring arity mismatch",
                              stmt.tuple_pattern->line,
                              stmt.tuple_pattern->column);
        }
    ...
    if (stmt.record_pattern) {
        ...
        if (rhs.type != DType::kRecord) {
            throw util::Error("Destructuring expects a record value",
                              stmt.record_pattern->line,
                              stmt.record_pattern->column);
        }
        if (rhs.record.fields.size() != stmt.record_pattern->fields.size()) {
            throw util::Error("Record destructuring field mismatch",
                              stmt.record_pattern->line,
                              stmt.record_pattern->column);
        }
        for (size_t i = 0; i < stmt.record_pattern->fields.size(); ++i) {
            ...
            if (rhs.record.index.find(key) == rhs.record.index.end()) {
                throw util::Error(
                    "Record key not found in destructuring: " + key,
                    stmt.record_pattern->line, stmt.record_pattern->column);
            }
            size_t idx = rhs.record.index[key];
            bind_value(stmt.record_pattern->fields[i].second,
                       rhs.record.fields[idx].second);
        }
        return ExecResult{rhs, ControlSignal::kNone};
    }
    ...
    if (stmt.annotation.type) {
        if (!ValueMatchesType(value, stmt.annotation.type->name)) {
            throw util::Error("Type mismatch for variable '" + stmt.name + "'",
                              stmt.line, stmt.column);
        }
...
ExecResult Evaluator::EvaluateExpressionStatement(
...
std::string Value::ToString() const {
    switch (type) {
        ...
        case DType::kC128: {
            std::ostringstream oss;
            oss << std::setprecision(10) << complex.real() << "+"
                << complex.imag() << "i";
            return oss.str();
        }
        case DType::kDecimal: {
        ...
        case DType::kTensor: {
            ...
            auto render_dense = [&](const Value& dense) -> std::string {
                ...
                std::function<void(size_t, int64_t)> rec = [&](size_t dim,
                                                               int64_t offset) {
                ...
                if (!dense.tensor.shape.empty())
                    rec(0, 0);
                return out.str();
            };
            constexpr int64_t kMaxElemsToPrint = 64;
            if (tensor.kind == TensorKind::kDense &&
                tensor.size <= kMaxElemsToPrint) {
                oss << " " << render_dense(*this);
                return oss.str();
            }
            if (tensor.kind == TensorKind::kSparseCSR ||
                tensor.kind == TensorKind::kSparseCOO) {
                oss << " nnz=" << tensor.sparse_values.size();
                if (tensor.sparse_values.size() <=
                    static_cast<size_t>(kMaxElemsToPrint)) {
                    oss << " values=[";
                    if (tensor.kind == TensorKind::kSparseCSR) {
                        for (size_t i = 0; i < tensor.sparse_values.size();
                             ++i) {
                            if (i > 0)
                                oss << ", ";
                            oss << "(" << tensor.indices[i] << ":"
            ...
            if (tensor.kind == TensorKind::kRagged) {
                oss << " rows="
                    << (tensor.row_splits.empty()
                            ? 0
                            : tensor.row_splits.size() - 1);
                if (tensor.ragged_values.size() <=
                    static_cast<size_t>(kMaxElemsToPrint)) {
                    oss << " values=[";
        ...
        case DType::kRecord: {
            ...
            for (size_t i = 0; i < record.fields.size(); ++i) {
                if (i > 0)
                    oss << ", ";
                oss << record.fields[i].first << ": "
                    << record.fields[i].second.ToString();
            }
            oss << "}";
            return oss.str();
        }
