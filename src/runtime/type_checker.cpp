...
std::optional<Type> LookupDType(const std::string& name) {
    ...
    if (name == "i8")
        return Type{DType::kI8};
    if (name == "i16")
        return Type{DType::kI16};
    if (name == "i32")
        return Type{DType::kI32};
    if (name == "i64")
        return Type{DType::kI64};
    if (name == "u8")
        return Type{DType::kU8};
    if (name == "u16")
        return Type{DType::kU16};
    if (name == "u32")
        return Type{DType::kU32};
    if (name == "u64")
        return Type{DType::kU64};
    if (name == "f16")
        return Type{DType::kF16};
    if (name == "bfloat16")
        return Type{DType::kBF16};
    if (name == "f32")
        return Type{DType::kF32};
    if (name == "f64")
        return Type{DType::kF64};
    if (name == "complex64")
        return Type{DType::kC64};
    if (name == "complex128")
        return Type{DType::kC128};
    if (name == "string")
        return Type{DType::kString};
    if (name == "decimal")
        return Type{DType::kDecimal};
    if (name == "rational")
        return Type{DType::kRational};
    if (name == "tensor")
...
std::string TypeNameStr(const Type& t) {
    ...
    if (t.kind == DType::kRecord) {
        ...
        for (size_t i = 0; i < t.record_fields.size(); ++i) {
            ...
            s += field.first + ": " +
                 (field.second.has_value() ? DTypeName(field.second.value())
                                           : "dynamic");
        }
        s += "}";
        return s;
...
bool IsAssignableD(const std::optional<DType>& from,
                   const std::optional<DType>& to) {
    if (!to.has_value())
        return true;
    if (!from.has_value())
        return true;
...
bool IsAssignableType(const Type& from, const Type& to) {
    if (to.kind == DType::kTuple && from.kind == DType::kTuple) {
        if (to.tuple_elems.size() != from.tuple_elems.size())
            return false;
        for (size_t i = 0; i < to.tuple_elems.size(); ++i) {
            if (!IsAssignableD(from.tuple_elems[i], to.tuple_elems[i]))
                return false;
        }
        return true;
    }
    if (to.kind == DType::kRecord && from.kind == DType::kRecord) {
        if (to.record_fields.empty())
            return true;  // treat empty metadata as wildcard record
        if (to.record_fields.size() != from.record_fields.size())
            return false;
        for (size_t i = 0; i < to.record_fields.size(); ++i) {
            if (to.record_fields[i].first != from.record_fields[i].first)
                return false;
            if (!IsAssignableD(from.record_fields[i].second,
                               to.record_fields[i].second))
                return false;
        }
        return true;
    }
    if (to.kind == DType::kTensor) {
        if (from.kind != DType::kTensor)
            return false;
...
TypeChecker::TypeChecker() {
    ...
    functions_["clamp"] =
        FunSig{{std::nullopt, std::nullopt, std::nullopt}, std::nullopt};
    functions_["min"] = FunSig{{std::nullopt, std::nullopt}, std::nullopt};
    functions_["max"] = FunSig{{std::nullopt, std::nullopt}, std::nullopt};
    functions_["set_decimal_precision"] =
        FunSig{{Type{DType::kI32}}, std::nullopt};
    ...
    functions_["rational"] =
        FunSig{{std::nullopt, std::nullopt}, Type{DType::kRational}};
    functions_["complex"] =
        FunSig{{std::nullopt, std::nullopt}, Type{DType::kC128}};
    auto tensor_type = [&](TensorKind k,
                           std::optional<DType> elem = std::nullopt) {
        return Type::Tensor(k, elem);
    };
    functions_["tensor"] =
        FunSig{{std::nullopt, std::nullopt}, tensor_type(TensorKind::kDense)};
    functions_["tensor_values"] =
        FunSig{{std::nullopt}, tensor_type(TensorKind::kDense)};
    functions_["tensor_sparse_csr"] =
        FunSig{{std::nullopt, std::nullopt, std::nullopt, std::nullopt},
               tensor_type(TensorKind::kSparseCSR)};
    functions_["tensor_sparse_coo"] =
        FunSig{{std::nullopt, std::nullopt, std::nullopt, std::nullopt},
               tensor_type(TensorKind::kSparseCOO)};
    functions_["tensor_ragged"] =
        FunSig{{std::nullopt, std::nullopt}, tensor_type(TensorKind::kRagged)};
    functions_["to_dense"] =
        FunSig{{Type{DType::kTensor}}, tensor_type(TensorKind::kDense)};
    functions_["to_sparse_csr"] =
        FunSig{{Type{DType::kTensor}}, tensor_type(TensorKind::kSparseCSR)};
    functions_["to_sparse_coo"] =
        FunSig{{Type{DType::kTensor}}, tensor_type(TensorKind::kSparseCOO)};
    ...
    functions_["transpose"] =
        FunSig{{Type{DType::kTensor}}, tensor_type(TensorKind::kDense)};
    functions_["matmul"] = FunSig{{Type{DType::kTensor}, Type{DType::kTensor}},
                                  tensor_type(TensorKind::kDense)};
    functions_["solve"] = FunSig{{Type{DType::kTensor}, Type{DType::kTensor}},
                                 tensor_type(TensorKind::kDense)};
    ...
    functions_["conv2d"] = FunSig{{Type{DType::kTensor}, Type{DType::kTensor}},
                                  tensor_type(TensorKind::kDense)};
    functions_["max_pool2d"] =
        FunSig{{Type{DType::kTensor}, Type{DType::kI64}, Type{DType::kI64}},
               tensor_type(TensorKind::kDense)};
    functions_["fft1d"] = FunSig{{Type{DType::kTensor}}, Type{DType::kTuple}};
    functions_["philox"] =
        FunSig{{Type{DType::kU64}, Type{DType::kU64}, Type{DType::kU64}},
               Type{DType::kF64}};
    functions_["threefry"] =
        FunSig{{Type{DType::kU64}, Type{DType::kU64}, Type{DType::kU64}},
               Type{DType::kF64}};
    ...
    functions_["beta"] =
        FunSig{{std::nullopt, std::nullopt}, Type{DType::kF64}};
    functions_["erf"] = FunSig{{std::nullopt}, Type{DType::kF64}};
    functions_["erfc"] = FunSig{{std::nullopt}, Type{DType::kF64}};
    functions_["igamma"] =
        FunSig{{std::nullopt, std::nullopt}, Type{DType::kF64}};
    functions_["normal_pdf"] =
        FunSig{{std::nullopt, std::nullopt, std::nullopt}, Type{DType::kF64}};
    functions_["normal_cdf"] =
        FunSig{{std::nullopt, std::nullopt, std::nullopt}, Type{DType::kF64}};
    functions_["normal_sample"] =
        FunSig{{std::nullopt, std::nullopt, Type{DType::kU64},
                Type{DType::kU64}, Type{DType::kU64}},
               Type{DType::kF64}};
    functions_["uniform_pdf"] =
        FunSig{{std::nullopt, std::nullopt, std::nullopt}, Type{DType::kF64}};
    functions_["uniform_cdf"] =
        FunSig{{std::nullopt, std::nullopt, std::nullopt}, Type{DType::kF64}};
    functions_["uniform_sample"] = FunSig{
        {std::nullopt, std::nullopt, Type{DType::kU64}, Type{DType::kU64}},
        Type{DType::kF64}};
    functions_["exponential_pdf"] =
        FunSig{{std::nullopt, std::nullopt}, Type{DType::kF64}};
    functions_["exponential_cdf"] =
        FunSig{{std::nullopt, std::nullopt}, Type{DType::kF64}};
    functions_["exponential_sample"] =
        FunSig{{std::nullopt, Type{DType::kU64}, Type{DType::kU64}},
               Type{DType::kF64}};
    functions_["poisson_pmf"] =
        FunSig{{std::nullopt, std::nullopt}, Type{DType::kF64}};
    ...
    functions_["binomial_pmf"] =
        FunSig{{std::nullopt, std::nullopt, std::nullopt}, Type{DType::kF64}};
    functions_["binomial_sample"] = FunSig{
        {std::nullopt, std::nullopt, Type{DType::kU64}, Type{DType::kU64}},
        Type{DType::kI64}};
    functions_["quantile"] =
        FunSig{{Type{DType::kTensor}, std::nullopt}, Type{DType::kF64}};
    functions_["correlation"] =
        FunSig{{Type{DType::kTensor}, Type{DType::kTensor}}, Type{DType::kF64}};
    functions_["regression"] = FunSig{
        {Type{DType::kTensor}, Type{DType::kTensor}}, Type{DType::kTuple}};
    ...
    functions_["has_key"] = FunSig{{Type{DType::kRecord}, Type{DType::kString}},
                                   Type{DType::kBool}};
}
...
bool TypeChecker::IsAssignable(const std::optional<Type>& from,
                               const std::optional<Type>& to) {
    if (!to.has_value())
        return true;
    if (!from.has_value())
        return true;
...
std::optional<Type> TypeChecker::TypeOf(const parser::Expression* expr) {
    ...
    if (auto bin = dynamic_cast<const BinaryExpression*>(expr)) {
        ...
        if (lt->kind == DType::kTensor || rt->kind == DType::kTensor) {
            // Tensor mixing rules.
            auto kind_of =
                [&](const std::optional<Type>& t) -> std::optional<TensorKind> {
                return t.has_value() ? t->tensor_kind : std::nullopt;
            };
            auto elem_of =
                [&](const std::optional<Type>& t) -> std::optional<DType> {
                if (!t.has_value())
                    return std::nullopt;
                if (t->kind == DType::kTensor)
                    return t->tensor_elem;
                return t->kind;
            };
            auto promote_elem =
                [&](std::optional<DType> a,
                ...
                if (!b.has_value())
                    return a;
                return PromoteType(a.value(), b.value(), bin->line,
                                   bin->column);
            };
            auto lk = kind_of(lt);
            auto rk = kind_of(rt);
            ...
            if (lk.has_value() && rk.has_value()) {
                if (lk.value() == TensorKind::kRagged ||
                    rk.value() == TensorKind::kRagged) {
                    if (lk.value() != TensorKind::kRagged ||
                        rk.value() != TensorKind::kRagged) {
                        throw util::Error(
                            "Ragged tensors only operate with matching ragged "
                            "tensors",
                            line, col);
                    }
                ...
                if ((lk.value() == TensorKind::kDense &&
                     rk.value() != TensorKind::kDense) ||
                    (rk.value() == TensorKind::kDense &&
                     lk.value() != TensorKind::kDense)) {
                    Type t = Type::Tensor(TensorKind::kDense, elem);
                    return t;
                }
                if (lk.value() != rk.value()) {
                    throw util::Error(
                        "Sparse tensor formats must match; convert first", line,
                        col);
                }
        ...
        if (bin->op == BinaryOp::kEq || bin->op == BinaryOp::kNe ||
            bin->op == BinaryOp::kGt || bin->op == BinaryOp::kGe ||
            bin->op == BinaryOp::kLt || bin->op == BinaryOp::kLe) {
            return Type{DType::kBool};
        }
    ...
    if (auto call = dynamic_cast<const CallExpression*>(expr)) {
        if (call->callee == "cast") {
            ...
            if (type_id == nullptr) {
                throw util::Error(
                    "cast first argument must be a type name identifier",
                    call->line, call->column);
            }
        ...
        auto promote_opt = [&](std::optional<DType> a,
                               std::optional<DType> b) -> std::optional<DType> {
            if (!a.has_value() && !b.has_value())
                return std::nullopt;
            if (!a.has_value())
        ...
        auto elem_from_tuple =
            [&](const std::optional<Type>& t) -> std::optional<DType> {
            if (!t.has_value() || t->kind != DType::kTuple)
                return std::nullopt;
            std::optional<DType> elem;
            for (const auto& e : t->tuple_elems) {
                ...
                if (e.value() == DType::kTuple || e.value() == DType::kRecord ||
                    e.value() == DType::kFunction ||
                    e.value() == DType::kTensor) {
                    return std::nullopt;
                }
        ...
        auto elem_from_values =
            [&](const std::vector<std::optional<Type>>& args,
                size_t start) -> std::optional<DType> {
            std::optional<DType> elem;
            for (size_t i = start; i < args.size(); ++i) {
                if (!args[i].has_value())
                    return std::nullopt;
                if (args[i]->kind == DType::kTensor)
                    return std::nullopt;
        ...
        if (call->callee == "tensor") {
            if (call->args.size() < 2) {
                throw util::Error(
                    "tensor expects at least shape and fill value", call->line,
                    call->column);
            }
            std::optional<DType> elem;
        ...
        if (fn != functions_.end()) {
            ...
            if (sig.params.size() != call->args.size()) {
                throw util::Error(call->callee + " expects " +
                ...
                if (!IsAssignable(arg_types[i], sig.params[i])) {
                    throw util::Error(
                        "Type mismatch for argument " + std::to_string(i + 1) +
                            " to " + call->callee + " (expected " +
                            OptTypeName(sig.params[i]) + ", got " +
                            OptTypeName(arg_types[i]) + ")",
                        call->line, call->column);
                }
            }

            if (call->callee == "tensor_sparse_csr" ||
                call->callee == "tensor_sparse_coo") {
                std::optional<DType> elem = elem_from_tuple(
                    arg_types.size() > 3 ? arg_types[3] : std::nullopt);
                TensorKind kind = call->callee == "tensor_sparse_csr"
                                      ? TensorKind::kSparseCSR
                                      : TensorKind::kSparseCOO;
                return tensor_type(kind, elem);
            }
            if (call->callee == "tensor_ragged") {
                std::optional<DType> elem = elem_from_tuple(
                    arg_types.size() > 1 ? arg_types[1] : std::nullopt);
            ...
            if (call->callee == "to_dense" || call->callee == "to_sparse_csr" ||
                ...
                if (call->callee == "to_sparse_csr")
                    kind = TensorKind::kSparseCSR;
                if (call->callee == "to_sparse_coo")
                    kind = TensorKind::kSparseCOO;
                std::optional<DType> elem;
            ...
            if (call->callee == "sum" || call->callee == "mean" ||
                call->callee == "var" || call->callee == "std") {
                if (!arg_types.empty() && arg_types[0].has_value()) {
                    ...
                    throw util::Error(
                        call->callee + " expects a tensor or numeric value",
                        call->line, call->column);
                }
                return std::nullopt;
            }
            ...
            if (call->callee == "mod") {
                if (arg_types.size() >= 2 && arg_types[0].has_value() &&
                    arg_types[1].has_value()) {
                    DType a = arg_types[0]->kind;
                    DType b = arg_types[1]->kind;
                    if (IsNumericKind(a) && IsNumericKind(b)) {
                        if (IsFloatKind(a) || IsFloatKind(b) ||
                            IsComplexKind(a) || IsComplexKind(b) ||
                            a == DType::kDecimal || a == DType::kRational ||
                            b == DType::kDecimal || b == DType::kRational) {
                            return Type{DType::kF64};
                        }
                        return Type{DType::kI64};
                    }
            ...
            if (call->callee == "min" || call->callee == "max") {
                if (arg_types.size() >= 2 && arg_types[0].has_value() &&
                    arg_types[1].has_value()) {
                    return Type{PromoteType(arg_types[0]->kind,
                                            arg_types[1]->kind, call->line,
                                            call->column)};
                }
                return std::nullopt;
            }
            if (call->callee == "clamp") {
                if (arg_types.size() >= 3 && arg_types[0].has_value() &&
                    arg_types[1].has_value() && arg_types[2].has_value()) {
                    auto t0 =
                        PromoteType(arg_types[0]->kind, arg_types[1]->kind,
                                    call->line, call->column);
                    auto t1 = PromoteType(t0, arg_types[2]->kind, call->line,
                                          call->column);
                    return Type{t1};
                }
                return std::nullopt;
            ...
            if (call->callee == "matmul" || call->callee == "solve" ||
                call->callee == "conv2d") {
                std::optional<DType> elem;
                if (arg_types.size() >= 2 && arg_types[0].has_value() &&
                    arg_types[1].has_value()) {
                    elem = promote_opt(arg_types[0]->tensor_elem,
                                       arg_types[1]->tensor_elem);
                }
                return tensor_type(TensorKind::kDense, elem);
            }
            ...
            if (call->callee == "len") {
                if (!arg_types.empty() && arg_types[0].has_value()) {
                    ...
                    if (k != DType::kTensor && k != DType::kTuple &&
                        k != DType::kRecord) {
                        throw util::Error(
                            "len expects tuple, record, or tensor", call->line,
                            call->column);
                    }
                }
                return Type{DType::kI64};
            }
            if (call->callee == "keys" || call->callee == "values" ||
                call->callee == "has_key") {
                if (!arg_types.empty() && arg_types[0].has_value() &&
                    arg_types[0]->kind != DType::kRecord) {
                    throw util::Error(call->callee + " expects a record",
                                      call->line, call->column);
                }
                if (call->callee == "has_key") {
                    if (arg_types.size() > 1 && arg_types[1].has_value() &&
                        arg_types[1]->kind != DType::kString) {
                        throw util::Error("has_key expects a string key",
                                          call->line, call->column);
                    }
                    return Type{DType::kBool};
                }
    ...
    if (auto tuple = dynamic_cast<const TupleLiteral*>(expr)) {
        ...
        for (const auto& e : tuple->elements) {
            ...
            t.tuple_elems.push_back(et ? std::optional<DType>(et->kind)
                                       : std::nullopt);
        }
        return t;
    }
    if (auto record = dynamic_cast<const RecordLiteral*>(expr)) {
        ...
        for (const auto& f : record->fields) {
            ...
            t.record_fields.emplace_back(
                f.first, ft ? std::optional<DType>(ft->kind) : std::nullopt);
        }
        return t;
    }
    if (auto idx = dynamic_cast<const IndexExpression*>(expr)) {
        ...
        if (obj_t->kind == DType::kTuple) {
            if (auto num =
                    dynamic_cast<const NumberLiteral*>(idx->index.get())) {
                int64_t pos = static_cast<int64_t>(num->value);
                if (pos >= 0 &&
                    pos < static_cast<int64_t>(obj_t->tuple_elems.size())) {
                    const auto& elem =
                        obj_t->tuple_elems[static_cast<size_t>(pos)];
                    return elem.has_value()
                               ? std::optional<Type>{Type{elem.value()}}
                               : std::nullopt;
                }
            }
            return std::nullopt;
        ...
        if (obj_t->kind == DType::kRecord) {
            if (auto key =
                    dynamic_cast<const StringLiteral*>(idx->index.get())) {
                for (const auto& f : obj_t->record_fields) {
                    if (f.first == key->value) {
                        return f.second.has_value()
                                   ? std::optional<Type>{Type{f.second.value()}}
                                   : std::nullopt;
                    }
                }
                throw util::Error("Record key not found: " + key->value,
                                  idx->index->line, idx->index->column);
            }
            return std::nullopt;
        }
        return std::nullopt;
...
void TypeChecker::CheckStatement(parser::Statement* stmt) {
    ...
    if (auto* ifs = dynamic_cast<IfStatement*>(stmt)) {
        ...
        if (cond_t.has_value() && cond_t->kind != DType::kBool) {
            throw util::Error("Condition must be bool", ifs->condition->line,
                              ifs->condition->column);
        }
        CheckStatement(ifs->then_branch.get());
        if (ifs->else_branch)
            CheckStatement(ifs->else_branch.get());
        return;
    }
    if (auto* ws = dynamic_cast<WhileStatement*>(stmt)) {
        ...
        if (cond_t.has_value() && cond_t->kind != DType::kBool) {
            throw util::Error(
                "Condition must be bool (got " + OptTypeName(cond_t) + ")",
    ...
    if (auto* fs = dynamic_cast<ForStatement*>(stmt)) {
        ...
        if (fs->increment)
            CheckStatement(fs->increment.get());
        if (fs->body)
            CheckStatement(fs->body.get());
        ExitScope();
        return;
    }
        ...
        if (expected_return_.has_value()) {
            if (ret->expr) {
                ...
                if (!IsAssignable(rt, expected_return_)) {
                    throw util::Error("Return type mismatch (expected " +
                                          OptTypeName(expected_return_) +
                                          ", got " + OptTypeName(rt) + ")",
                                      ret->line, ret->column);
                }
            } else {
                throw util::Error("Missing return value", ret->line,
                                  ret->column);
            }
        }
        return;
    ...
    if (auto* asn = dynamic_cast<AssignmentStatement*>(stmt)) {
        ...
        if (asn->tuple_pattern) {
            if (asn->annotation.type) {
                throw util::Error(
                    "Annotations are not supported on destructuring "
                    "assignments",
                    asn->line, asn->column);
            }
            if (!val_t.has_value() || val_t->kind != DType::kTuple) {
                throw util::Error("Tuple destructuring expects a tuple value",
                                  asn->tuple_pattern->line,
        ...
        if (asn->record_pattern) {
            ...
            if (!val_t.has_value() || val_t->kind != DType::kRecord) {
                throw util::Error("Record destructuring expects a record value",
                                  asn->record_pattern->line,
                                  asn->record_pattern->column);
            }
            if (val_t->record_fields.size() !=
                asn->record_pattern->fields.size()) {
            ...
            for (const auto& field : asn->record_pattern->fields) {
                ...
                if (!found) {
                    throw util::Error(
                        "Record key not found in destructuring: " + field.first,
        ...
        if (annot.has_value()) {
            if (!IsAssignable(val_t, annot)) {
                throw util::Error("Type mismatch in assignment to " +
                                      asn->name + " (expected " +
                                      OptTypeName(annot) + ", got " +
                                      OptTypeName(val_t) + ")",
                                  asn->line, asn->column);
            }
    ...
    if (dynamic_cast<BreakStatement*>(stmt) ||
        dynamic_cast<ContinueStatement*>(stmt)) {
        return;
    }
