...
LaunchConfig Make2DLaunch(uint64_t width, uint64_t height) {
    ...
    cfg.block[1] = 16;
    cfg.grid[0] =
        static_cast<uint32_t>((width + cfg.block[0] - 1) / cfg.block[0]);
    cfg.grid[1] =
        static_cast<uint32_t>((height + cfg.block[1] - 1) / cfg.block[1]);
    cfg.use_local = true;
    return cfg;
}
...
struct GpuArg {
    ...
    static GpuArg Value(const void* data, size_t size) {
        ...
        arg.value.assign(static_cast<const uint8_t*>(data),
...
class GpuExecutor {
   public:
        ...
        switch (buffer.backend) {
            case BackendType::kOpenCL:
                return opencl_->WriteBuffer(buffer.device_index, buffer.opencl,
                                            data, bytes);
            case BackendType::kCUDA:
                return cuda_->WriteBuffer(buffer.device_index, buffer.cuda,
                                          data, bytes);
            case BackendType::kHIP:
                return hip_->WriteBuffer(buffer.device_index, buffer.hip, data,
                                         bytes);
            case BackendType::kMetal:
#if defined(__APPLE__)
                return metal_->WriteBuffer(buffer.device_index, buffer.metal,
                                           data, bytes);
#else
                return Status::Unavailable(
                    "Metal backend unavailable on this platform");
        ...
        switch (buffer.backend) {
            case BackendType::kOpenCL:
                return opencl_->ReadBuffer(buffer.device_index, buffer.opencl,
                                           data, bytes);
            case BackendType::kCUDA:
                return cuda_->ReadBuffer(buffer.device_index, buffer.cuda, data,
                                         bytes);
            case BackendType::kHIP:
                return hip_->ReadBuffer(buffer.device_index, buffer.hip, data,
                                        bytes);
            case BackendType::kMetal:
#if defined(__APPLE__)
                return metal_->ReadBuffer(buffer.device_index, buffer.metal,
                                          data, bytes);
#else
                return Status::Unavailable(
                    "Metal backend unavailable on this platform");
    ...
    Status WriteBufferFromDouble(const GpuBuffer& buffer,
                                 const double* data,
                                 size_t count) {
        if (use_fp64_) {
    ...
    StatusOr<std::vector<double>> ReadBufferToDouble(const GpuBuffer& buffer,
                                                     size_t count) {
        std::vector<double> out(count, 0.0);
        if (use_fp64_) {
            Status status =
                ReadBuffer(buffer, out.data(), count * sizeof(double));
            if (!status.ok())
                return status;
            return out;
        }
    ...
    Status LaunchKernel(const GpuKernel& kernel,
        ...
        switch (kernel.backend) {
            case BackendType::kOpenCL: {
                ...
                for (const auto& arg : args) {
                    if (arg.kind == GpuArg::Kind::kBuffer) {
                        oargs.push_back(
                            OpenCLKernelArg::Mem(arg.buffer->opencl.mem));
                    } else {
                        oargs.push_back(OpenCLKernelArg::Value(
                            arg.value.data(), arg.value.size()));
                    }
                }
            ...
            case BackendType::kCUDA: {
                ...
                for (const auto& arg : args) {
                    if (arg.kind == GpuArg::Kind::kBuffer) {
                        cargs.push_back(
                            CudaKernelArg::Device(arg.buffer->cuda.ptr));
                    } else {
                        cargs.push_back(CudaKernelArg::Value(arg.value.data(),
                                                             arg.value.size()));
                    }
                }
            ...
            case BackendType::kHIP: {
                ...
                for (const auto& arg : args) {
                    if (arg.kind == GpuArg::Kind::kBuffer) {
                        hargs.push_back(
                            HipKernelArg::Device(arg.buffer->hip.ptr));
                    } else {
                        hargs.push_back(HipKernelArg::Value(arg.value.data(),
                                                            arg.value.size()));
                    }
                }
            ...
            case BackendType::kMetal: {
                ...
                for (const auto& arg : args) {
                    if (arg.kind == GpuArg::Kind::kBuffer) {
                        margs.push_back(
                            MetalKernelArg::Buffer(arg.buffer->metal.handle));
                    } else {
                        margs.push_back(MetalKernelArg::Value(
                            arg.value.data(), arg.value.size()));
                    }
                }
   ...
   private:
    ...
    std::string KernelPath(const std::string& file_base) const {
        ...
        return (std::filesystem::path(kernel_dir_) / (file_base + ext))
            .string();
    }

    std::string KernelCacheKey(const std::string& name) const {
        std::ostringstream key;
        key << BackendTypeName(backend_type_) << ":" << device_index_ << ":"
            << name;
        return key.str();
    }
...
void LogGpuError(BackendType backend,
                 const Status& status,
                 const std::string& op) {
...
std::optional<Value> BuildDenseTensor(const std::vector<int64_t>& shape,
    ...
    if (static_cast<size_t>(out.tensor.size) != data.size())
        return std::nullopt;
...
std::optional<Value> TryGpuElemwise(const Value& lhs,
                                    const Value& rhs,
                                    parser::BinaryOp op,
                                    int line,
                                    int column,
    ...
    if ((lhs.type == DType::kTensor &&
        ...
        if (lhs.type != DType::kTensor || rhs.type != DType::kTensor ||
            ...
            rhs.tensor.kind != TensorKind::kRagged) {
            throw util::Error(
                "Ragged tensors only support ops with matching ragged tensors "
                "(use to_dense)",
                line, column);
        }
        ...
        if (lhs.tensor.ragged_values.size() !=
            ...
            throw util::Error("Ragged value buffers must match in length", line,
                              column);
        }
        ...
        auto lhs_buf_or = exec.AllocateBuffer(
            count * (exec.UseFp64() ? sizeof(double) : sizeof(float)));
        if (!lhs_buf_or.ok()) {
            LogGpuError(exec.Type(), lhs_buf_or.status(), "elemwise");
            if (error)
                *error = lhs_buf_or.status().message;
            return std::nullopt;
        }
        auto rhs_buf_or = exec.AllocateBuffer(
            count * (exec.UseFp64() ? sizeof(double) : sizeof(float)));
        if (!rhs_buf_or.ok()) {
            LogGpuError(exec.Type(), rhs_buf_or.status(), "elemwise");
            if (error)
                *error = rhs_buf_or.status().message;
            return std::nullopt;
        }
        auto out_buf_or = exec.AllocateBuffer(
            count * (exec.UseFp64() ? sizeof(double) : sizeof(float)));
        if (!out_buf_or.ok()) {
            LogGpuError(exec.Type(), out_buf_or.status(), "elemwise");
            if (error)
                *error = out_buf_or.status().message;
            return std::nullopt;
        }
        ...
        std::vector<double> lhs_vals(lhs.tensor.ragged_values.begin(),
                                     lhs.tensor.ragged_values.end());
        std::vector<double> rhs_vals(rhs.tensor.ragged_values.begin(),
                                     rhs.tensor.ragged_values.end());
        Status status = exec.WriteBufferFromDouble(lhs_buf, lhs_vals.data(),
                                                   lhs_vals.size());
        if (!status.ok()) {
        ...
        status = exec.WriteBufferFromDouble(rhs_buf, rhs_vals.data(),
                                            rhs_vals.size());
        if (!status.ok()) {
            LogGpuError(exec.Type(), status, "elemwise");
            if (error)
    ...
    Value rhs_d = rhs;
    bool lhs_sparse =
        lhs.type == DType::kTensor && lhs.tensor.kind != TensorKind::kDense;
    bool rhs_sparse =
    ...
    if (lhs_sparse || rhs_sparse) {
        if (lhs.type == DType::kTensor && rhs.type == DType::kTensor &&
            ...
            if (lhs.tensor.shape != rhs.tensor.shape) {
                throw util::Error(
                    "Sparse tensors must share shape for elementwise ops", line,
                    column);
            }
            sparse_out = true;
            ...
            throw util::Error("Sparse tensor formats must match; convert first",
                              line, column);
        }
    ...
    std::vector<int64_t> lhs_shape = lhs_d.type == DType::kTensor
                                         ? lhs_d.tensor.shape
                                         : std::vector<int64_t>{};
    std::vector<int64_t> rhs_shape = rhs_d.type == DType::kTensor
                                         ? rhs_d.tensor.shape
                                         : std::vector<int64_t>{};
    ...
    if (out_shape.size() > kMaxTensorDims) {
        if (error)
            *error = "tensor rank exceeds GPU broadcast limit";
        return std::nullopt;
    }

    DType elem_target;
    if (lhs_d.type == DType::kTensor && rhs_d.type == DType::kTensor) {
        elem_target = PromoteType(lhs_d.tensor.elem_type,
                                  rhs_d.tensor.elem_type, line, column);
    } else if (lhs_d.type == DType::kTensor) {
        elem_target =
            PromoteType(lhs_d.tensor.elem_type, rhs_d.type, line, column);
    } else {
        elem_target =
    ...
    std::vector<int64_t> lhs_bstrides =
        lhs_d.type == DType::kTensor
            ? BroadcastStrides(lhs_d.tensor.shape, lhs_d.tensor.strides,
                               out_shape.size())
            : std::vector<int64_t>(out_shape.size(), 0);
    std::vector<int64_t> rhs_bstrides =
        rhs_d.type == DType::kTensor
            ? BroadcastStrides(rhs_d.tensor.shape, rhs_d.tensor.strides,
                               out_shape.size())
            : std::vector<int64_t>(out_shape.size(), 0);

    if (lhs_bstrides.size() > kMaxTensorDims ||
        rhs_bstrides.size() > kMaxTensorDims ||
    ...
    size_t lhs_count = lhs_d.type == DType::kTensor
                           ? static_cast<size_t>(lhs_d.tensor.size)
                           : 1;
    size_t rhs_count = rhs_d.type == DType::kTensor
                           ? static_cast<size_t>(rhs_d.tensor.size)
                           : 1;
    ...
    if (lhs_d.type == DType::kTensor) {
        Status status =
            exec.WriteBufferFromDouble(lhs_buf, lhs_d.tensor.Data(), lhs_count);
        if (!status.ok()) {
            LogGpuError(exec.Type(), status, "elemwise");
            if (error)
                *error = status.message;
    ...
    if (rhs_d.type == DType::kTensor) {
        Status status =
            exec.WriteBufferFromDouble(rhs_buf, rhs_d.tensor.Data(), rhs_count);
        if (!status.ok()) {
            LogGpuError(exec.Type(), status, "elemwise");
            if (error)
                *error = status.message;
    ...
    auto out_dense =
        BuildDenseTensor(out_shape, elem_target, out_data_or.value());
...
std::optional<Value> TryGpuReduce(const Value& v,
                                  ReduceKind kind,
                                  int line,
                                  int column,
                                  std::string* error) {
    ...
    if (!in_buf_or.ok()) {
        ...
        if (error)
            *error = in_buf_or.status().message;
        return std::nullopt;
    }
    ...
    Status status =
        exec.WriteBufferFromDouble(in_buf, dense.tensor.Data(), count);
    if (!status.ok()) {
...
std::optional<Value> TryGpuTranspose(const Value& v,
                                     int line,
                                     int column,
                                     std::string* error) {
    auto& exec = GpuExecutor::Instance();
    ...
    auto out_dense = BuildDenseTensor({cols, rows}, dense.tensor.elem_type,
                                      out_data_or.value());
    if (!out_dense.has_value())
        return std::nullopt;
    return out_dense.value();
...
std::optional<Value> TryGpuMatmul(const Value& lhs,
                                  const Value& rhs,
                                  int line,
                                  int column,
                                  std::string* error) {
...
std::optional<Value> TryGpuConv2d(const Value& input,
    ...
    Status status =
        exec.WriteBufferFromDouble(in_buf, in.tensor.Data(), in_count);
    if (!status.ok()) {
...
std::optional<Value> TryGpuMaxPool2d(const Value& input,
                                     int64_t k_h,
                                     int64_t k_w,
                                     int line,
                                     int column,
    ...
    if (k_h <= 0 || k_w <= 0) {
        throw util::Error("max_pool2d kernel sizes must be positive", line,
                          column);
    }
    int64_t h = in.tensor.shape[0];
    int64_t w = in.tensor.shape[1];
    ...
    auto out_dense =
        BuildDenseTensor({oh, ow}, in.tensor.elem_type, out_data_or.value());
    if (!out_dense.has_value())
        return std::nullopt;
    return out_dense.value();
}

std::optional<Value> TryGpuFft1d(const Value& input,
                                 int line,
                                 int column,
                                 std::string* error) {
    auto& exec = GpuExecutor::Instance();
    ...
    if (!real_buf_or.ok()) {
        ...
        if (error)
            *error = real_buf_or.status().message;
        return std::nullopt;
    }
    ...
    if (!imag_buf_or.ok()) {
        ...
        if (error)
            *error = imag_buf_or.status().message;
        return std::nullopt;
    }
...
std::optional<Value> TryGpuSolve(const Value& a,
                                 const Value& b,
                                 int line,
                                 int column,
                                 std::string* error) {
    ...
    if (A.tensor.shape.size() != 2) {
        throw util::Error("solve expects a 2D coefficient matrix", line,
                          column);
    }
    ...
    if (B.tensor.shape.size() == 1) {
        if (B.tensor.shape[0] != n) {
            throw util::Error("solve rhs length must match matrix rows", line,
                              column);
        }
        rhs_cols = 1;
    } else if (B.tensor.shape.size() == 2) {
        if (B.tensor.shape[0] != n) {
            throw util::Error("solve rhs rows must match matrix rows", line,
                              column);
        }
    ...
    if (!scratch_or.ok()) {
        ...
        if (error)
            *error = scratch_or.status().message;
        return std::nullopt;
    }
    ...
    if (!status_or.ok()) {
        ...
        if (error)
            *error = status_or.status().message;
        return std::nullopt;
    }
...
std::optional<Value> TryGpuLu(const Value& a,
                              int line,
                              int column,
                              std::string* error) {
    auto& exec = GpuExecutor::Instance();
    ...
    if (!l_val.has_value() || !u_val.has_value())
        return std::nullopt;
    return Value::Tuple({l_val.value(), u_val.value()});
}

std::optional<Value> TryGpuQr(const Value& a,
                              int line,
                              int column,
                              std::string* error) {
    auto& exec = GpuExecutor::Instance();
    ...
    if (!q_val.has_value() || !r_val.has_value())
        return std::nullopt;
    return Value::Tuple({q_val.value(), r_val.value()});
}

std::optional<Value> TryGpuSvd(const Value& a,
                               int line,
                               int column,
                               std::string* error) {
    auto& exec = GpuExecutor::Instance();
    ...
    int64_t n = A.tensor.shape[1];
    if (m != 2 || n != 2) {
        throw util::Error("svd currently supports 2x2 matrices only", line,
                          column);
    }
    ...
    if (!u_val.has_value() || !s_val.has_value() || !v_val.has_value())
        return std::nullopt;
    return Value::Tuple({u_val.value(), s_val.value(), v_val.value()});
}

std::optional<Value> TryGpuQuantile(const Value& data,
                                    double q,
                                    int line,
                                    int column,
                                    std::string* error) {
    ...
    Status status =
        exec.WriteBufferFromDouble(in_buf, dense.tensor.Data(), in_count);
    if (!status.ok()) {
...
std::optional<Value> TryGpuCorrelation(const Value& x,
                                       const Value& y,
                                       int line,
                                       int column,
                                       std::string* error) {
    ...
    if (x.type != DType::kTensor || y.type != DType::kTensor) {
        throw util::Error("correlation requires equal non-empty vectors", line,
                          column);
    }
    ...
    if (status_code != 0) {
        throw util::Error("correlation undefined (zero variance)", line,
                          column);
    }
...
std::optional<Value> TryGpuRegression(const Value& x,
                                      const Value& y,
                                      int line,
                                      int column,
                                      std::string* error) {
    ...
    if (!exec.Enabled())
        return std::nullopt;
    if (x.type != DType::kTensor || y.type != DType::kTensor) {
        throw util::Error("regression requires equal non-empty vectors", line,
                          column);
    }
    Value dx = ToDenseTensor(x, line, column);
    Value dy = ToDenseTensor(y, line, column);
