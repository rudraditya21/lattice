...
bool TraceKernelSource(const KernelTrace& trace, std::string* out_path) {
    ...
    const std::filesystem::path backend_dir =
        root / BackendTypeName(trace.backend);
    std::error_code ec;
    ...
    const std::string filename =
        name + "_" + std::to_string(counter) + ".trace";
    const std::filesystem::path path = backend_dir / filename;
    std::ofstream out(path);
    if (!out)
        return false;
    out << "# backend=" << BackendTypeName(trace.backend) << "\n";
    if (trace.device_index >= 0)
        out << "# device_index=" << trace.device_index << "\n";
    if (!trace.device_name.empty())
        out << "# device_name=" << trace.device_name << "\n";
    if (!trace.kernel_name.empty())
        out << "# kernel=" << trace.kernel_name << "\n";
    if (!trace.build_options.empty())
        out << "# build_options=" << trace.build_options << "\n";
    out << trace.source;
    if (out_path)
