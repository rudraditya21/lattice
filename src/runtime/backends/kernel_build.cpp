...
void AppendDefine(std::ostringstream& out, const std::string& prefix, const std::string& name,
...
std::string KernelDefineString(const KernelBuildDefines& defs, const std::string& prefix) {
...
std::string LoadBuildOptionsEnv(const std::string& backend_env) {
  ...
  if (global.empty()) return backend;
  if (backend.empty()) return global;
