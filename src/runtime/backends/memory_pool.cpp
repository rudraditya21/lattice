...
MemoryPoolConfig LoadMemoryPoolConfig(const std::string& prefix,
                                      MemoryPoolConfig base) {
    ...
    if (ParseCountValue(std::getenv(EnvKey(prefix, "MAX_ENTRIES").c_str()),
                        &parsed)) {
        base.max_pool_entries = parsed;
    }
    if (ParseSizeValue(std::getenv(EnvKey(prefix, "MAX_ENTRY_BYTES").c_str()),
                       &parsed)) {
        base.max_entry_bytes = parsed;
    }
    if (ParseSizeValue(std::getenv(EnvKey(prefix, "BUCKET_BYTES").c_str()),
                       &parsed)) {
...
void AccumulateMemoryPoolStats(MemoryPoolStats* dst,
                               const MemoryPoolStats& src) {
    if (!dst)
        return;
    ...
    dst->peak_in_use_bytes =
        std::max(dst->peak_in_use_bytes, src.peak_in_use_bytes);
    dst->peak_in_use_blocks =
        std::max(dst->peak_in_use_blocks, src.peak_in_use_blocks);
}

MemoryPool::MemoryPool(std::string label,
                       MemoryPoolConfig config,
                       AllocFn alloc_fn,
                       FreeFn free_fn,
                       ScrubFn scrub_fn)
...
StatusOr<PoolBlock> MemoryPool::Acquire(size_t bytes, size_t alignment) {
    ...
    if (block.key == 0) {
        return Status::Internal(label_ +
                                " pool allocation returned null handle");
    }
...
bool MemoryPool::ShouldPool(size_t bytes) const {
    return bytes > 0 && bytes <= config_.max_entry_bytes &&
           config_.max_pool_bytes > 0 && config_.max_pool_entries > 0;
}

void MemoryPool::UpdatePeakLocked() {
    stats_.peak_in_use_bytes =
        std::max(stats_.peak_in_use_bytes, stats_.in_use_bytes);
    stats_.peak_in_use_blocks =
        std::max(stats_.peak_in_use_blocks, stats_.in_use_blocks);
}
