...
DeviceKind ParseKind(const char* value) {
    ...
    if (v == "accel" || v == "accelerator")
        return DeviceKind::kAccelerator;
    return DeviceKind::kAny;
}

bool MatchesPatterns(const DeviceIdentity& dev,
    ...
    for (const auto& pattern : patterns) {
        ...
        if (name.find(pattern) != std::string::npos ||
            vendor.find(pattern) != std::string::npos ||
...
std::vector<int> Reorder(const std::vector<int>& selected,
...
DeviceSelectionOptions LoadDeviceSelectionOptions(const char* prefix) {
    ...
    options.include_patterns =
        SplitPatterns(GetEnvWithFallback(prefix, "_DEVICE_INCLUDE"));
    options.exclude_patterns =
        SplitPatterns(GetEnvWithFallback(prefix, "_DEVICE_EXCLUDE"));
    ...
    options.explicit_selection =
        options.kind != DeviceKind::kAny || !options.include_patterns.empty() ||
...
DeviceSelectionResult SelectDevices(const std::vector<DeviceIdentity>& devices,
    ...
    for (const auto& dev : devices) {
        if (options.kind != DeviceKind::kAny && dev.kind != options.kind)
            continue;
        if (!options.include_patterns.empty() &&
            !MatchesPatterns(dev, options.include_patterns)) {
            continue;
        }
        if (!options.exclude_patterns.empty() &&
            MatchesPatterns(dev, options.exclude_patterns)) {
            continue;
        }
        if (!options.indices.empty() &&
            std::find(options.indices.begin(), options.indices.end(),
                      dev.index) == options.indices.end()) {
            continue;
        }
        if (!options.mask.empty() &&
            mask_set.find(dev.index) == mask_set.end()) {
            continue;
        }
