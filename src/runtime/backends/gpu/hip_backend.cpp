...
Status HipStatus(StatusCode code,
    ...
    void Record() override {
        ...
        if (!event_) {
            ...
            if (loader_->hipEventCreateWithFlags) {
                if (loader_->hipEventCreateWithFlags(&event_, flags) !=
                    gpu::hipSuccess) {
                    event_ = nullptr;
                }
...
class HipStream final : public Stream {
   public:
    ...
    void AddDependency(const std::shared_ptr<Event>& ev) override {
        ...
        if (hip_event && loader_ && loader_->hipStreamWaitEvent &&
            hip_event->handle()) {
...
StatusOr<Allocation> HipBackend::AllocatePinned(size_t bytes,
                                                size_t alignment) const {
    Status status = EnsureInitialized();
    if (!status.ok())
        return status;
...
std::vector<DeviceMemoryStats> HipBackend::MemoryStatsByDevice() const {
    ...
    for (const auto& dev : devices_) {
        ...
        entry.backend = BackendType::kHIP;
...
StatusOr<uint64_t> HipBackend::ElapsedNs(
    ...
    float ms = 0.0f;
    gpu::hipError_t err =
        loader_.hipEventElapsedTime(&ms, start_ev->handle(), end_ev->handle());
    if (err != gpu::hipSuccess) {
...
StatusOr<HipBuffer> HipBackend::CreateBuffer(int device_index,
                                             size_t bytes) const {
    Status status = EnsureInitialized();
    if (!status.ok())
        return status;
    ...
    if (!block_or.ok())
        return block_or.status();
    auto block = block_or.value();
    return HipBuffer{reinterpret_cast<void*>(block.handle), bytes,
                     device_index};
}
...
Status HipBackend::WriteBuffer(int device_index,
                               const HipBuffer& buffer,
                               const void* data,
                               size_t bytes,
                               size_t offset) const {
    ...
    auto dst = reinterpret_cast<char*>(buffer.ptr) +
...
Status HipBackend::ReadBuffer(int device_index,
                              const HipBuffer& buffer,
                              void* data,
                              size_t bytes,
                              size_t offset) const {
    ...
    auto src = reinterpret_cast<const char*>(buffer.ptr) +
               static_cast<std::ptrdiff_t>(offset);
    gpu::hipError_t err =
...
StatusOr<std::vector<HipKernel>> HipBackend::BuildKernelsFromFile(
    ...
    if (!ReadFile(source_path, &source, &error)) {
        return HipStatus(StatusCode::kInvalidArgument, BackendErrorKind::kIo,
                         error);
    }
    ...
    for (size_t i = 0; i < devices_.size(); ++i) {
        ...
        gpu::hipError_t err = loader_.hipModuleGetFunction(
            &func, module_or.value(), kernel_name.c_str());
        if (err != gpu::hipSuccess) {
            last_error = "hipModuleGetFunction failed: " +
                         gpu::HipErrorString(err, &loader_);
            continue;
        }
        HipKernel handle;
    ...
    if (out.empty()) {
        if (last_error.empty())
            last_error = "No HIP kernels built";
        return HipStatus(StatusCode::kUnavailable, BackendErrorKind::kBuild,
                         last_error);
    }
    return out;
...
Status HipBackend::SmokeTest() const {
    ...
    std::unordered_map<int, HipKernel> mul_by_device;
    ...
    for (const auto& kernel : add_kernels) {
        ...
        if (it == mul_by_device.end()) {
            ...
            return HipStatus(StatusCode::kUnavailable,
                             ...
                             "Missing HIP vec_mul kernel");
        }
        const HipKernel& mul_kernel = it->second;
        auto buf_a_or =
        ...
        for (size_t i = 0; i < kCount; ++i) {
            if (out[i] != a[i] + b[i]) {
                return HipStatus(StatusCode::kInternal,
                                 BackendErrorKind::kRuntime,
        ...
        for (size_t i = 0; i < kCount; ++i) {
            if (out[i] != a[i] * b[i]) {
                return HipStatus(StatusCode::kInternal,
                                 BackendErrorKind::kRuntime,
                                 "HIP smoke test failed: vec_mul mismatch");
            }
        }
...
Status HipBackend::EnsureInitialized() const {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (loader_.hipDeviceGet(&dev.device, i) != gpu::hipSuccess)
            continue;
        char name[256] = {0};
        ...
        if (loader_.hipDeviceTotalMem) {
            size_t total_mem = 0;
            if (loader_.hipDeviceTotalMem(&total_mem, dev.device) ==
                gpu::hipSuccess) {
        ...
        if (loader_.hipDeviceGetAttribute) {
            int max_threads = 0;
            if (loader_.hipDeviceGetAttribute(&max_threads,
                                              kAttrMaxThreadsPerBlock,
                                              dev.device) == gpu::hipSuccess) {
                dev.caps.max_threads_per_block =
                    static_cast<size_t>(max_threads);
                dev.caps.max_work_group_size = dev.caps.max_threads_per_block;
            }
            int shared_mem = 0;
            if (loader_.hipDeviceGetAttribute(&shared_mem,
                                              kAttrSharedMemPerBlock,
                                              dev.device) == gpu::hipSuccess) {
                dev.caps.shared_mem_bytes = static_cast<size_t>(shared_mem);
            }
            int clock = 0;
            if (loader_.hipDeviceGetAttribute(&clock, kAttrClockRate,
                                              dev.device) == gpu::hipSuccess) {
                dev.desc.clock_khz = clock;
            }
            int mp = 0;
            if (loader_.hipDeviceGetAttribute(&mp, kAttrMultiprocessorCount,
                                              dev.device) == gpu::hipSuccess) {
        ...
        dev.caps.quirks = QueryDeviceQuirks(BackendType::kHIP, dev.desc.vendor,
    ...
    for (int idx : selected.indices) {
        if (idx < 0 || idx >= static_cast<int>(candidates.size()))
            continue;
        if (!candidate_valid[static_cast<size_t>(idx)])
            continue;
        ...
        if (loader_.hipCtxCreate) {
            if (loader_.hipCtxCreate(&dev.context, 0, dev.device) !=
                gpu::hipSuccess) {
                LogBackend({LogLevel::kWarn, BackendType::kHIP,
    ...
    if (devices_.empty()) {
        init_status_ =
            HipStatus(StatusCode::kUnavailable, BackendErrorKind::kContext,
...

    {
        ...
        CacheStore& cache_store = HipCacheStore();
        for (const auto& dev : devices_) {
...
MemoryPool* HipBackend::PinnedPool() const {
    ...
    pinned_pool_ = std::make_unique<MemoryPool>("hip_pinned_pool", config,
                                                alloc_fn, free_fn, scrub_fn);
    return pinned_pool_.get();
}
...
std::string HipBackend::CacheKey(const DeviceContext& dev,
    ...
    return "hip_" + meta_hash + "_" + src_hash;
}
...
StatusOr<gpu::hipModule_t> HipBackend::BuildOrLoadModule(
    ...
    auto load_module =
        [&](const std::string& image) -> StatusOr<gpu::hipModule_t> {
    ...
    if (source_is_binary) {
        ...
        gpu::HiprtcResult rc = loader_.hiprtcCreateProgram(
            &prog, source.c_str(), "lattice.hip", 0, nullptr, nullptr);
        if (rc != 0) {
            return HipStatus(StatusCode::kInternal, BackendErrorKind::kCompile,
        ...
        rc = loader_.hiprtcCompileProgram(
