...
#elif defined(__APPLE__)
    const char* libs[] = {"libnvrtc.dylib",
                          "/usr/local/cuda/lib/libnvrtc.dylib"};
#else
    const char* libs[] = {"libnvrtc.so", "libnvrtc.so.1", "libnvrtc.so.11.2",
...
bool CudaLoader::Load(std::string* error) {
    ...
    LoadOptional(driver, "cuDeviceComputeCapability",
    ...
    ok &=
        LoadSymbol(driver, "cuModuleGetFunction", &cuModuleGetFunction, error);
    ...
    if (LoadNvrtcLib(&nvrtc)) {
        ...
        LoadSymbol(nvrtc, "nvrtcGetProgramLogSize", &nvrtcGetProgramLogSize,
                   nullptr);
