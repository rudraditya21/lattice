...
DeviceMetadata BuildDeviceMetadata(const CudaDeviceDesc& desc,
    ...
    meta.device_version =
        std::to_string(desc.major) + "." + std::to_string(desc.minor);
...
class CudaStream final : public Stream {
   public:
    ...
    void AddDependency(const std::shared_ptr<Event>& ev) override {
        ...
        if (cuda_event && loader_ && loader_->cuStreamWaitEvent &&
            cuda_event->handle()) {
...
StatusOr<Allocation> CudaBackend::AllocatePinned(size_t bytes,
                                                 size_t alignment) const {
    Status status = EnsureInitialized();
    if (!status.ok())
        return status;
...
size_t CudaBackend::OutstandingAllocs() const {
    ...
    if (pinned_pool_)
        total += pinned_pool_->Outstanding();
    return total;
}
...
StatusOr<uint64_t> CudaBackend::ElapsedNs(
    ...
    float ms = 0.0f;
    gpu::CUresult err =
        loader_.cuEventElapsedTime(&ms, start_ev->handle(), end_ev->handle());
...
StatusOr<CudaBuffer> CudaBackend::CreateBuffer(int device_index,
                                               size_t bytes) const {
    Status status = EnsureInitialized();
    if (!status.ok())
        return status;
    ...
    if (!block_or.ok())
        return block_or.status();
    auto block = block_or.value();
    return CudaBuffer{static_cast<gpu::CUdeviceptr>(block.handle), bytes,
                      device_index};
}
...
Status CudaBackend::WriteBuffer(int device_index,
...
Status CudaBackend::ReadBuffer(int device_index,
                               const CudaBuffer& buffer,
                               void* data,
                               size_t bytes,
                               size_t offset) const {
...
StatusOr<std::vector<CudaKernel>> CudaBackend::BuildKernelsFromFile(
    ...
    if (!ReadFile(source_path, &source, &error)) {
        return CudaStatus(StatusCode::kInvalidArgument, BackendErrorKind::kIo,
                          error);
    }
    ...
    for (size_t i = 0; i < devices_.size(); ++i) {
        ...
        auto module_or =
            BuildOrLoadModule(dev, source, build_options, cache_key,
                              source_is_binary, kernel_name);
        if (!module_or.ok()) {
            last_error = module_or.status().message;
            continue;
        }
        gpu::CUfunction func = nullptr;
        gpu::CUresult err = loader_.cuModuleGetFunction(
            &func, module_or.value(), kernel_name.c_str());
        if (err != gpu::CUDA_SUCCESS) {
            last_error = "cuModuleGetFunction failed: " +
                         gpu::CudaErrorString(err, &loader_);
            continue;
        }
        CudaKernel handle;
    ...
    if (out.empty()) {
        if (last_error.empty())
            last_error = "No CUDA kernels built";
        return CudaStatus(StatusCode::kUnavailable, BackendErrorKind::kBuild,
                          last_error);
    }
    return out;
...
Status CudaBackend::LaunchKernel(const CudaKernel& kernel,
    ...
    for (const auto& arg : args) {
        if (arg.kind == CudaKernelArg::Kind::kDevicePtr) {
            ...
            value_args.emplace_back(
                reinterpret_cast<const uint8_t*>(arg.value),
    ...
    gpu::CUresult err = loader_.cuLaunchKernel(
        kernel.func, config.grid[0], config.grid[1], config.grid[2],
...
Status CudaBackend::SmokeTest() const {
    ...
    for (const auto& kernel : kernels) {
        ...
        cfg.block[0] = 256;
        cfg.grid[0] =
            static_cast<uint32_t>((kCount + cfg.block[0] - 1) / cfg.block[0]);
...
Status CudaBackend::EnsureInitialized() const {
    ...
    if (!loader_.Load(&error)) {
        init_status_ = CudaStatus(StatusCode::kUnavailable,
    ...
    if (!loader_.cuInit) {
        init_status_ =
            CudaStatus(StatusCode::kUnavailable, BackendErrorKind::kInit,
                       "CUDA driver missing cuInit");
        return init_status_;
    }
    ...
    if (runtime_str.empty())
        runtime_str = driver_str;

    int count = 0;
    err = loader_.cuDeviceGetCount(&count);
    if (err != gpu::CUDA_SUCCESS || count <= 0) {
        init_status_ =
            CudaStatus(StatusCode::kUnavailable, BackendErrorKind::kDiscovery,
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (loader_.cuDeviceGet(&dev.device, i) != gpu::CUDA_SUCCESS)
            continue;
        char name[256] = {0};
        ...
        if (loader_.cuDeviceComputeCapability) {
            loader_.cuDeviceComputeCapability(&dev.desc.major, &dev.desc.minor,
                                              dev.device);
        }
        if (loader_.cuDeviceTotalMem) {
            size_t total_mem = 0;
            if (loader_.cuDeviceTotalMem(&total_mem, dev.device) ==
                gpu::CUDA_SUCCESS) {
        ...
        if (loader_.cuDeviceGetAttribute) {
            int mp = 0;
            if (loader_.cuDeviceGetAttribute(&mp, kAttrMultiprocessorCount,
                                             dev.device) == gpu::CUDA_SUCCESS) {
                dev.desc.multiprocessor_count = mp;
            }
            int clock = 0;
            if (loader_.cuDeviceGetAttribute(&clock, kAttrClockRate,
                                             dev.device) == gpu::CUDA_SUCCESS) {
                dev.desc.clock_khz = clock;
            }
            int max_threads = 0;
            if (loader_.cuDeviceGetAttribute(&max_threads,
                                             kAttrMaxThreadsPerBlock,
                                             dev.device) == gpu::CUDA_SUCCESS) {
                dev.caps.max_threads_per_block =
                    static_cast<size_t>(max_threads);
                dev.caps.max_work_group_size = dev.caps.max_threads_per_block;
            }
            int shared_mem = 0;
            if (loader_.cuDeviceGetAttribute(&shared_mem,
                                             kAttrSharedMemPerBlock,
                                             dev.device) == gpu::CUDA_SUCCESS) {
        ...
        if (dev.desc.major == 0 && dev.desc.minor == 0) {
            ...
            dev.caps.fp64 = (dev.desc.major > 1 ||
                             (dev.desc.major == 1 && dev.desc.minor >= 3))
                                ? CapabilityStatus::kYes
                                : CapabilityStatus::kNo;
            dev.caps.fp16 = (dev.desc.major > 5 ||
                             (dev.desc.major == 5 && dev.desc.minor >= 3))
                                ? CapabilityStatus::kYes
                                : CapabilityStatus::kNo;
        }
        dev.caps.quirks = QueryDeviceQuirks(BackendType::kCUDA, dev.desc.vendor,
                                            dev.desc.name, driver_str);
        dev.caps.is_software =
            (dev.caps.quirks.flags & kSoftwareEmulation) != 0;
        if (dev.caps.quirks.flags & kDisableFp16)
            dev.caps.fp16 = CapabilityStatus::kNo;
        if (dev.caps.quirks.flags & kDisableFp64)
            dev.caps.fp64 = CapabilityStatus::kNo;
    ...
    DeviceSelectionOptions selection =
        LoadDeviceSelectionOptions("LATTICE_CUDA");
    DeviceSelectionResult selected = SelectDevices(identities, selection);
    if (selected.indices.empty()) {
        init_status_ =
            CudaStatus(StatusCode::kUnavailable, BackendErrorKind::kDiscovery,
                       selected.diagnostics.empty() ? "No CUDA devices selected"
    ...
    for (int idx : selected.indices) {
        ...
        if (dev.caps.quirks.disabled) {
            LogBackend({LogLevel::kWarn, BackendType::kCUDA,
                        BackendErrorKind::kDiscovery,
                        "skipping device: " + dev.caps.quirks.reason,
                        "device_skip", dev.desc.index, dev.desc.name});
            continue;
        }
        if (!loader_.cuCtxCreate)
            continue;
        if (loader_.cuCtxCreate(&dev.context, 0, dev.device) !=
            gpu::CUDA_SUCCESS) {
            LogBackend({LogLevel::kWarn, BackendType::kCUDA,
                        BackendErrorKind::kContext, "context init failed",
                        "context", dev.desc.index, dev.desc.name});
            continue;
        }
        if (loader_.cuCtxSetCurrent) {
            loader_.cuCtxSetCurrent(dev.context);
    ...
    if (devices_.empty()) {
        init_status_ =
            CudaStatus(StatusCode::kUnavailable, BackendErrorKind::kContext,
...
MemoryPool* CudaBackend::PinnedPool() const {
    ...
    auto alloc_fn = [this](size_t bytes,
                           size_t alignment) -> StatusOr<PoolBlock> {
        (void)alignment;
    ...
    pinned_pool_ = std::make_unique<MemoryPool>("cuda_pinned_pool", config,
                                                alloc_fn, free_fn, scrub_fn);
    return pinned_pool_.get();
}
...
StatusOr<gpu::CUmodule> CudaBackend::BuildOrLoadModule(
    ...
    auto load_module =
        [&](const std::string& image) -> StatusOr<gpu::CUmodule> {
        if (loader_.cuCtxSetCurrent) {
            loader_.cuCtxSetCurrent(dev.context);
        }
        ...
        if (loader_.cuModuleLoadDataEx) {
            err = loader_.cuModuleLoadDataEx(&module, image.data(), 0, nullptr,
                                             nullptr);
        } else {
    ...
    if (source_is_binary) {
        ...
        if (!loader_.NvrtcLoaded()) {
            return CudaStatus(
                StatusCode::kUnavailable, BackendErrorKind::kBuild,
                "NVRTC not available for CUDA kernel compilation");
        }
        gpu::nvrtcProgram prog = nullptr;
        gpu::NvrtcResult rc = loader_.nvrtcCreateProgram(
            &prog, source.c_str(), "lattice.cu", 0, nullptr, nullptr);
        if (rc != 0) {
            return CudaStatus(StatusCode::kInternal, BackendErrorKind::kCompile,
                              "nvrtcCreateProgram failed: " +
                                  gpu::NvrtcErrorString(rc, &loader_));
        }
        ...
        rc = loader_.nvrtcCompileProgram(
            prog, static_cast<int>(opt_ptrs.size()),
        ...
        if (rc != 0) {
            return CudaStatus(
                StatusCode::kInternal, BackendErrorKind::kBuild,
