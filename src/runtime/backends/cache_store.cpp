...
std::unordered_map<std::string, std::string> ParseKeyValueTokens(
...
bool WriteFileAtomically(const std::filesystem::path& path,
    ...
    if (!out) {
        if (error)
            *error = "Failed to open file for write: " + tmp.string();
        return false;
    }
    ...
    if (!out) {
        if (error)
            *error = "Failed to write file: " + tmp.string();
        return false;
    }
    out.close();
    std::error_code ec;
    ...
    if (ec) {
        if (error)
            *error = "Failed to rename cache file: " + ec.message();
        return false;
    }
    return true;
}
...
CachePolicy LoadCachePolicyFromEnv() {
    ...
    if (const char* env = std::getenv("LATTICE_CACHE_MAX_AGE_DAYS")) {
        uint64_t value = 0;
        if (ParseUint64(env, &value))
            policy.max_age_seconds = value * 24ull * 60ull * 60ull;
    }
...
bool CacheStore::WriteBinary(const CacheKey& key,
                             const void* data,
                             size_t size,
                             std::string* error) {
    if (!policy_.enabled)
    ...
    if (ec) {
        if (error)
            *error = "Failed to create cache dir: " + ec.message();
        return false;
    }
    const std::filesystem::path path = EntryPath(key.key);
...
void CacheStore::Invalidate(const CacheKey& key) {
    ...
    if (it != entries_.end()) {
        if (!key.fingerprint.empty() &&
            it->second.fingerprint != key.fingerprint) {
            RemoveEntry(key.key);
            FlushIndex(nullptr);
            return;
        }
...
bool CacheStore::LoadIndex(std::string* error) {
    ...
    if (!in) {
        if (error)
            *error = "Failed to open cache index: " + index_path_.string();
        return false;
    }
    std::string line;
    if (!std::getline(in, line))
        return false;
    const auto header = SplitTokens(line);
    if (header.size() < 3 || header[0] != "lattice-cache-index")
        return false;
    const std::string expected_version =
    ...
    while (std::getline(in, line)) {
        ...
        it = kv.find("created");
        if (it != kv.end())
            entry.created = static_cast<uint64_t>(
                std::strtoull(it->second.c_str(), nullptr, 10));
        it = kv.find("accessed");
        if (it != kv.end())
            entry.accessed = static_cast<uint64_t>(
                std::strtoull(it->second.c_str(), nullptr, 10));
        if (entry.key.empty())
            continue;
...
bool CacheStore::FlushIndex(std::string* error) {
    ...
    for (const auto& kv : entries_) {
        ...
        out << "entry"
            << " key=" << entry.key << " fingerprint=" << entry.fingerprint
            << " size=" << entry.size << " created=" << entry.created
            << " accessed=" << entry.accessed << "\n";
    }
    const std::string content = out.str();
    return WriteFileAtomically(index_path_, content.data(), content.size(),
                               error);
}
    ...
    if ((policy_.max_bytes > 0 && total_bytes > policy_.max_bytes) ||
        ...
        while ((policy_.max_bytes > 0 && total_bytes > policy_.max_bytes) ||
               (policy_.max_entries > 0 &&
                entries_.size() > policy_.max_entries)) {
...
std::string DeviceFingerprint(const DeviceMetadata& meta) {
    std::ostringstream ss;
    ss << meta.backend << "|" << meta.vendor << "|" << meta.name << "|"
       << meta.driver_version << "|" << meta.runtime_version << "|"
       << meta.device_version << "|" << meta.platform_name << "|"
...
bool DeviceMetadataStore::Read(const std::string& fingerprint,
    ...
    const std::string expected_meta =
        "lattice-device-meta v" + std::to_string(kDeviceMetaVersion);
    ...
    while (std::getline(in, line)) {
        ...
        if (key == "backend")
            ...
            out.multiprocessor_count =
                ParseIntValue(value, out.multiprocessor_count);
...
std::filesystem::path DeviceMetadataStore::MetaPath(
...
bool DeviceMetadataStore::EnsureDir(std::string* error) {
    ...
    if (ec) {
        if (error)
            *error = "Failed to create device meta dir: " + ec.message();
        return false;
    }
    return true;
}
