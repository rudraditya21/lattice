...
const DeviceQuirkEntry kQuirkTable[] = {
    {BackendScope::kOpenCL, "portable computing language", "", "",
     kSoftwareEmulation, false, "OpenCL POCL (software)"},
...
DeviceQuirkInfo QueryDeviceQuirks(BackendType backend,
    ...
    if (!ignore_quirks) {
        for (const auto& entry : kQuirkTable) {
            if (!MatchesScope(entry.scope, backend))
                continue;
            if (!Contains(vendor_lc, entry.vendor_substr))
                continue;
            if (!Contains(name_lc, entry.name_substr))
                continue;
            if (!Contains(driver_lc, entry.driver_substr))
                continue;
            info.flags |= entry.flags;
            if (entry.disabled) {
                info.disabled = true;
                if (info.reason.empty())
                    info.reason = entry.reason ? entry.reason : "";
            }
        }
    }

    const char* disable_software =
        std::getenv("LATTICE_DISABLE_SOFTWARE_DEVICES");
    if (disable_software && disable_software[0] != '\0') {
        if (info.flags & kSoftwareEmulation) {
            info.disabled = true;
            if (info.reason.empty())
                info.reason = "software device disabled";
        }
    }

    const auto blacklist =
        SplitPatterns(std::getenv("LATTICE_DEVICE_BLACKLIST"));
