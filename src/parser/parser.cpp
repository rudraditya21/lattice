...
std::unique_ptr<Expression> Parser::ParseExpression() {
    ...
    if (Peek().type != lexer::TokenType::kEof) {
        throw util::Error("Unexpected tokens after expression", Peek().line,
                          Peek().column);
    }
    return expr;
}
...
std::unique_ptr<Statement> Parser::FunctionStatementRule() {
    ...
    if (!Match(lexer::TokenType::kRParen)) {
        while (true) {
            ...
            Consume(lexer::TokenType::kComma,
                    "Expected ',' between parameters");
        }
    }
    BindingAnnotation return_type;
    if (Peek().type == lexer::TokenType::kMinus &&
        Next().type == lexer::TokenType::kGreater) {
        Advance();  // '-'
        Advance();  // '>'
    ...
    auto stmt = std::make_unique<FunctionStatement>(
        std::move(name), std::move(params), std::move(param_types),
...
std::unique_ptr<Statement> Parser::ReturnStatementRule() {
    int line = Previous().line;
    int col = Previous().column;
    if (Peek().type == lexer::TokenType::kSemicolon ||
        Peek().type == lexer::TokenType::kRBrace) {
...
std::unique_ptr<Statement> Parser::ForStatementRule() {
    ...
    if (!Match(lexer::TokenType::kSemicolon)) {
        ...
        Consume(lexer::TokenType::kSemicolon,
    ...
    if (Match(lexer::TokenType::kRParen)) {
        ...
        Consume(lexer::TokenType::kRParen,
                "Expected ')' after for-loop increment");
    }

    auto body = StatementRule();
    auto stmt =
        std::make_unique<ForStatement>(std::move(init), std::move(condition),
...
std::unique_ptr<Statement> Parser::Block() {
    ...
    while (!Match(lexer::TokenType::kRBrace)) {
        if (Peek().type == lexer::TokenType::kEof) {
            throw util::Error("Unterminated block", Previous().line,
                              Previous().column);
        }
...
std::unique_ptr<Statement> Parser::AssignmentOrExpression() {
    if (Peek().type == lexer::TokenType::kIdentifier) {
        ...
        if (Next().type == lexer::TokenType::kEqual ||
            Next().type == lexer::TokenType::kColon) {
            is_assignment = true;
        }
        if (is_assignment) {
            ...
            auto stmt = std::make_unique<AssignmentStatement>(
                std::move(name), std::move(ann), std::move(value));
            stmt->line = line;
            stmt->column = col;
            return stmt;
        }
    ...
    if (Peek().type == lexer::TokenType::kLParen) {
        ...
        if (!Match(lexer::TokenType::kRParen)) {
            while (true) {
                Consume(lexer::TokenType::kIdentifier,
                        "Expected name in tuple destructuring");
        ...
        pattern->column = col;
        BindingAnnotation ann = ParseBindingAnnotation();
        Consume(lexer::TokenType::kEqual,
                "Expected '=' in destructuring assignment");
        auto value = ExpressionRule();
        auto stmt = std::make_unique<AssignmentStatement>(
            std::move(pattern), std::move(ann), std::move(value));
        stmt->line = line;
        stmt->column = col;
        return stmt;
    }
    if (Peek().type == lexer::TokenType::kLBrace) {
        ...
        if (!Match(lexer::TokenType::kRBrace)) {
            while (true) {
                Consume(lexer::TokenType::kIdentifier,
                        "Expected field name in record destructuring");
                ...
                if (Match(lexer::TokenType::kRBrace))
                    break;
                Consume(lexer::TokenType::kComma,
                        "Expected ',' in record destructuring");
            }
        }
    ...
    while (true) {
        if (Match(lexer::TokenType::kEqualEqual)) {
            ...
            expr = std::make_unique<BinaryExpression>(
                BinaryOp::kEq, std::move(expr), std::move(rhs));
            continue;
        }
        if (Match(lexer::TokenType::kBangEqual)) {
            auto rhs = Comparison();
            expr = std::make_unique<BinaryExpression>(
                BinaryOp::kNe, std::move(expr), std::move(rhs));
            continue;
        }
        break;
    }
...
std::unique_ptr<Expression> Parser::Comparison() {
    ...
    while (true) {
        if (Match(lexer::TokenType::kGreater)) {
            auto rhs = Term();
            expr = std::make_unique<BinaryExpression>(
                BinaryOp::kGt, std::move(expr), std::move(rhs));
            continue;
        }
        if (Match(lexer::TokenType::kGreaterEqual)) {
            auto rhs = Term();
            expr = std::make_unique<BinaryExpression>(
                BinaryOp::kGe, std::move(expr), std::move(rhs));
            continue;
        }
        if (Match(lexer::TokenType::kLess)) {
            auto rhs = Term();
            expr = std::make_unique<BinaryExpression>(
                BinaryOp::kLt, std::move(expr), std::move(rhs));
            continue;
        }
        if (Match(lexer::TokenType::kLessEqual)) {
            auto rhs = Term();
            expr = std::make_unique<BinaryExpression>(
                BinaryOp::kLe, std::move(expr), std::move(rhs));
            continue;
        }
        break;
    }
    ...
    while (true) {
        if (Match(lexer::TokenType::kPlus)) {
            ...
            auto bin = std::make_unique<BinaryExpression>(
                BinaryOp::kAdd, std::move(expr), std::move(rhs));
        ...
        if (Match(lexer::TokenType::kMinus)) {
            auto rhs = Factor();
            auto bin = std::make_unique<BinaryExpression>(
                BinaryOp::kSub, std::move(expr), std::move(rhs));
            bin->line = Previous().line;
            bin->column = Previous().column;
            expr = std::move(bin);
            continue;
...
std::unique_ptr<Expression> Parser::Factor() {
    ...
    while (true) {
        if (Match(lexer::TokenType::kStar)) {
            auto rhs = Unary();
            auto bin = std::make_unique<BinaryExpression>(
                BinaryOp::kMul, std::move(expr), std::move(rhs));
            bin->line = Previous().line;
            bin->column = Previous().column;
            expr = std::move(bin);
            continue;
        ...
        if (Match(lexer::TokenType::kSlash)) {
            auto rhs = Unary();
            auto bin = std::make_unique<BinaryExpression>(
                BinaryOp::kDiv, std::move(expr), std::move(rhs));
            bin->line = Previous().line;
            bin->column = Previous().column;
            expr = std::move(bin);
            continue;
...
std::unique_ptr<Expression> Parser::Unary() {
    if (Match(lexer::TokenType::kMinus)) {
        auto operand = Unary();
        auto ue = std::make_unique<UnaryExpression>(UnaryOp::kNegate,
                                                    std::move(operand));
...
std::unique_ptr<Expression> Parser::Primary() {
    ...
    if (Match(lexer::TokenType::kNumber)) {
        ...
        if (Match(lexer::TokenType::kLParen)) {
            ...
            auto call_expr =
                std::make_unique<CallExpression>(name, std::move(args));
        ...
        if (Match(lexer::TokenType::kRParen)) {
            throw util::Error("Empty tuple not allowed", Previous().line,
                              Previous().column);
        }
        ...
        if (!Match(lexer::TokenType::kRBrace)) {
            while (true) {
                if (!(Match(lexer::TokenType::kIdentifier) ||
                      Match(lexer::TokenType::kString))) {
                    throw util::Error("Expected field name in record",
                                      Peek().line, Peek().column);
                }
                std::string key = Previous().lexeme;
                Consume(lexer::TokenType::kColon,
        ...
        throw util::Error("Unexpected token: " + Peek().lexeme, Peek().line,
                          Peek().column);
    }
    ...
    while (Match(lexer::TokenType::kLBracket)) {
        ...
        expr = std::make_unique<IndexExpression>(
            std::move(expr), std::move(idx), Previous().line,
            Previous().column);
    }
    return expr;
}
...
std::unique_ptr<TypeName> Parser::ParseTypeName() {
    ...
    if (!dtype.has_value()) {
        throw util::Error("Unknown type: " + name, Previous().line,
                          Previous().column);
    }
